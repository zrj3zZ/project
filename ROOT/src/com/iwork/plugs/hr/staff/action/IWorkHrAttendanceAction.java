...
import com.iwork.plugs.hr.staff.service.IWorkHrAttendanceService;
import com.iwork.sdk.ProcessAPI;
import com.opensymphony.xwork2.ActionSupport;

public class IWorkHrAttendanceAction extends ActionSupport {
	
	private IWorkHrAttendanceService iWorkHrAttendanceService;
	...
	public String showAttendance(){
		...
		if (processFrom1 != null) {
			if (!processFrom1.equals("")) {
		...
		if (processFrom2 != null) {
			if (!processFrom2.equals("")) {
				cm = new ConditionModel();
				cm.setFieldName("SQRQ");
		...
		html = iWorkHrAttendanceService.showAttendanceHtml(list);
		total = ProcessAPI.getInstance().queryBDDataListSize(entityName, conditionList, orderByList);
		return SUCCESS;
	}
	
	public IWorkHrAttendanceService getiWorkHrAttendanceService() {
		return iWorkHrAttendanceService;
	}

	public void setiWorkHrAttendanceService(
			IWorkHrAttendanceService iWorkHrAttendanceService) {
		this.iWorkHrAttendanceService = iWorkHrAttendanceService;
	}
