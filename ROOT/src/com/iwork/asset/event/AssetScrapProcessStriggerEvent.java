 ...
 * 公共触发器
 * 归档触发器
 * 修改流程状态(使用中-已归档)使用状态改为报废状态
 * 修改资产卡片状态（已报废）
 * 在流程触发器中配置
 * @author zhangruibo
 *
 */
public class AssetScrapProcessStriggerEvent extends ProcessTriggerEvent{
	public static final String zckpsubformkey="SUBFORM_GDZCZQGZBD";//固定资产卡片subformkey
	public AssetScrapProcessStriggerEvent(UserContext uc, HashMap<String,Object> hash) {
		super(uc, hash);
	...
	public boolean execute(){
		...
		/**以下要实现的业务是将报废流程中的详细信息插入到资源卡片中  2016-1-20-**/
		...
		if(dataMap!=null){
			...
			//管理状态（已报废）
			hash.put("STATUS", StatusConstant.ZYKP_GLZTWBF);
			//获取当前系统时间
			...
			//操作
			hashMap.put("ACTIONTYPE",StatusConstant.ZYKP_GLZTWBF);
			//添加管理变更日期
			hashMap.put("RQ", date);
			//获取部门
