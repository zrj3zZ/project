...
<title>IWORK综合应用管理系统</title> 
 ...
 //==========================装载快捷键===============================//快捷键
			...
			var setting = {
					...
					async: {
						enable: true, 
						url:"openPurviewProcess_treejson.action", 
						dataType:"json", 
						dataFilter: filter,
						autoParam:["id","type"]
					},callback: {
 		...
 		function onClick(event, treeId, treeNode, clickFlag){
 			var zTree = $.fn.zTree.getZTreeObj("purviewTree");
 				...
 				if(type=='group'){
 					...
 					var url = "openUnProcessPurviewList.action?actdefid="+treeNode.actDefId;
 					var setting2 = {
					check: {
						enable: true
					},
				...
				$("#actdefid").val(treeNode.actDefId); 
				$.fn.zTree.init($("#purviewListTree"), setting2); 
 			 }
 		}
			...
			if(type=='category'){
				...
				function setpurview(){
					...
					var zTree = $.fn.zTree.getZTreeObj("purviewListTree");
		 			...
		 			$("#purviewIds").val(str);
					$.post('openPurviewProcess_setProcesslist.action',$("#editForm").serialize(),function(data){
				    	if(data=='success'){
				    		alert("授权成功");
				    	}else{
				    		alert("授权异常,请稍后再试");
