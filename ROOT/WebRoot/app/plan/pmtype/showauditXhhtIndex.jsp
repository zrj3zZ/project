...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>     		
	...
	var selectedNodes;
		...
		function backAudit(id){
			var instanceid = $("#instanceid").val();
			//alert(instanceid);
			var pageUrl = "edit_option_action.action?instanceid="+instanceid+"&rowid="+id;
		...
		function audit(){
			var instanceid = $("#instanceid").val();
			var checkeds = $("input:name=xm_item");
			alert(checkeds);
			retuen;
			var ids = [];
	...
	<div >
		<div class="tools_nav">
			...
			<table width="100%">
				<TR>
					<td></td>
					<td  style="text-align:right" id="tjsp"></td>
				</TR>
			</table>
			</div>
		 </div>
	   ...
	   <div style="overflow:auto">
		...
		<table WIDTH="100%" style="border:1px solid #efefef">
			...
			<s:iterator value="children"  id="inner" >
				...
				<s:if test="#inner.BBZT=='A2'">
				<input name='xm_item' type="hidden" value='#inner.id' id='xm_item'/>
				</s:if>
				</div></td>
				...
				<TD ><input type="text" name="BYYS" onChange="setbyys(<s:property value="#inner.id"/>,<s:property value="#inner.NDYSYE"/>,this)" id="BYYS_<s:property value="#inner.id"/>" style="width:58px" value="<s:property value="#inner.BYYS"/>"/></TD>
				
				
				<TD style="display:none"><s:property value="#inner.BBZT"/></TD>
				
				<s:if test="#inner.BBZT=='A2'">
					<TD><s:select list="fklyList" name="FKLY" id="FKLY_%{#inner.id}" theme="simple" onChange="setfkly('%{#inner.id}',this)" listKey="FKLY" listValue="FKLY" value="#inner.FKLY" headerKey="0" headerValue="请选择"></s:select></TD>
				</s:if>
				
					
				<s:if test="#inner.BBZT=='A2'">
					<TD ><a  href="javascript:backAudit(<s:property value="#inner.id"/>)">驳回</a></TD>
				</s:if>
				<s:else>
					<TD ></TD>
				</s:else>
