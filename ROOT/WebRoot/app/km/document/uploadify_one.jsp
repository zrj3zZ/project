...
<html>
		...
		$(document).ready(function() {
		  ...
		  $('#uploadify').uploadify({
		    ...
		    'onAllComplete':function(event,data) {    								        
		    						...
		    						$.post('km_file_version_update.action',{fileUUID:fileUUID,fileSize:fileSize,id:id},function(data){
		    						        if(data=='ok'){
		    ...
		    'onCancel'    : function(event,ID,fileObj,data) {  	    					
    							...
    							for(var i=0;i<fileArray.length;i++){
										...
										if(ID==obj.id){
											...
											if(flag){
											    $.post('km_file_version_update_undo.action',{fileUUID:obj.uuid},function(response){  
		    						        		if(response=='ok'){
  ...
  <div region="center"  style="border:0px;padding:0px;background:#gegege" >
  	<form action="uploadifyUpload.action" method="post" enctype="multipart/form-data">
        ...
        <s:hidden id="id" name="id"></s:hidden>                      <!-- 需要更新版本的文档的id -->
    </form>
</div>
<div region="south" split="true" style="border:0px;height:60px;text-align:right;color:#666"> 
    <div style='text-align:left;align:center;color:#ff0000;margin-left:5px;'>
        ...
        注意事项：只能上传一个文件，且文件大小不超过&nbsp;<B><s:property value='sizeLimit'/>B</B>
    </div> 
    <a class="easyui-linkbutton" icon="icon-ok" href="javascript:uploadify_one()">确定上传</a> 
