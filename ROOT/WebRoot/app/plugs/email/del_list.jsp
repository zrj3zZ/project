...
<title>IWORK综合应用管理系统</title>
	...
	$(function(){
		$("#pp").pagination({
			    ...
			    onSelectPage:function(pageNumber, pageSize){
			    	var pageUrl="iwork_mail_del_list.action?pageSize="+pageSize+"&pageNumber="+pageNumber;
		   ...
		   var zhuangtais = "";
			if(checks.length>0){
				...
				if(letterIds==""){
					...
					var url = 'iwork_mail_restar_star.action?letterIds='+letterIds+'&zhuangtais='+encodeURI(zhuangtais);
					var option={
							type: "post",
							url: url,
							data: "",
	...
	//转发
	function forward(){
	   var counts = document.getElementsByName('chk_list');			
			var replyId = "";
	...
	//双击邮件时触发的事件
	 function showDetailInfo(id,title,taskid,zhuangtai){
      		if(title.length>5){
      			title = title.substring(0,5)+"...";
      		}
      		var pageurl="iwork_email_read.action?mailId="+id+"&taskid="+taskid+"&Ownerid="+zhuangtai;
...
<body class="easyui-layout">
		 <div region="north" border="false" > 
		 	<div class="tools_nav" style="padding-left:2px;">
		 		...
		 		<input type="button" name="removeBtn" onclick="javascript:forward();" value="转发"/>
		 		<input type="button" name="removeBtn" onclick="javascript:Star();" value="标星"/>
            ...
            <div region="center" border="false" style="background: #fff; border:1px solid #fff">
            	<table width="100%" cellspacing="0" cellpadding="0" border="0">
            		<tr>
		 			  ...
		 			  <s:iterator value="mailDelLists" status="status">
		 			  		<tr <s:if test="read==1">class="mailItem"</s:if><s:if test="read==0">class="mailItem unread"</s:if>  name="receiveMail">
				 				<td ><input name="chk_list" type="checkbox" value="<s:property value="id"/>"/></td>
				 				<td  class="mailicon" onClick="javascript:showDetailInfo('<s:property value="bindid"/>','<s:property value="title"/>','<s:property value="id"/>','<s:property value="owner"/>');">
				 				<s:if test="read==1"><img id="bflag" src="iwork_img/sysletter/mail_have_readed.png"/></s:if>
				 				<s:if test="read==0"><img id="bflag" src="iwork_img/sysletter/mail_no_readed.png"/></s:if></td></td>
				 				<td  class="mailUser" onClick="javascript:showDetailInfo('<s:property value="bindid"/>','<s:property value="title"/>','<s:property value="id"/>','<s:property value="owner"/>');"><s:property value="owner"/></td>
				 				<td  class="mailTitle" onClick="javascript:showDetailInfo('<s:property value="bindid"/>','<s:property value="title"/>','<s:property value="id"/>','<s:property value="owner"/>');" title="<s:property value="titles"/>"><s:property value="title"/><span> - 请查看一下</span></td>
				 				<td  class="mailtime" onClick="javascript:showDetailInfo('<s:property value="bindid"/>','<s:property value="title"/>','<s:property value="id"/>','<s:property value="owner"/>');"><s:property value="%{getText('{0,date,yyyy-MM-dd HH:mm}',{createTime})}"/></td>
				 				...
				 				<input id="taskid" type="hidden" value="<s:property value="id"/>"/>
				 				<input id="bindid" type="hidden" value="<s:property value="bindid"/>"/>
				 				<input id="type" type="hidden" value="<s:property value="type"/>"/>
				 				<input id="clickStars" type="hidden" value="<s:property value="isstar"/>"/>
				 			</tr>
		 			</s:iterator>
		 			
		 			<s:if test="mailDelLists==null||mailDelLists.size()==0"> 
		 					<tr class="mailItem ">
