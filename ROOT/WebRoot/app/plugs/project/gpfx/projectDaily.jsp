...
<script type="text/javascript">
	$(function() {
	    ...
	    $("#search").click(function(){
	        ...
	        var enddate = $("#ENDDATE").val();
	      	var seachUrl = encodeURI("zqb_gpfx_project_showDaily.action?startdate="+startdate+"&enddate="+enddate+"&projectNo="+projectno);
	...
	function editDaily(instanceid){
		var formid=$("#formid").val();
		var id=$("#xmrbid").val();
		var pageUrl = "url:openFormPage.action?formid="+formid+"&demId="+id+"&instanceId=" + instanceid;
		$.dialog({ 
			title:'项目日报表单',
			loadingText:'正在加载中,请稍后...',
			bgcolor:'#999',
	...
	function removeDaily(instanceid) {
		if (confirm('确定删除此项目日报吗？')) {
			var pageUrl = "zqb_gpfx_project_removeDaily.action?instanceid="+ instanceid;
	...
	function expPro(){
		...
		var projectno=$("#projectNo").val();
		var pageUrl = "zqb_gpfx_project_daily.action?projectNo="+projectno;
				...
				<s:iterator value="list" status="status">
					<tr class="cell">
						...
						<td>&nbsp;<a href="javascript:void(0)"
							onclick="editDaily(<s:property value='INSTANCEID'/>)">编辑</a>&nbsp;|&nbsp;<a href="javascript:void(0)"
							onclick="removeDaily(<s:property value='INSTANCEID'/>)">删除</a></td>
						</td> 
						</td> 
					</tr>
				</s:iterator>
