	...
	<script type="text/javascript">
		...
		function initTree(){
			var setting = {
						...
						async: {
							enable: true, 
							url:encodeURI("multibook_orgjson.action?parentDept=<s:property value='parentDept' escapeHtml='false'/>&currentDept=<s:property value='currentDept' escapeHtml='false'/>&startDept=<s:property value='startDept' escapeHtml='false'/>&input=<s:property value="input"/>"), 
							dataType:"json",
			...
			function insertData(){
				...
				//var zTree = $.fn.zTree.getZTreeObj("multitree");
	 			...
	 			for(var i=0;i<nodes.length;i++){
	 				...
	 				pushData(userIdObj,targetUserId,nodes[i].userId);
	 				pushData(userNoObj,targetUserNo,nodes[i].userno);
	 				pushData(userNameObj,targetUserName,nodes[i].userName);
	 				pushData(deptIdObj,targetDeptId,nodes[i].deptId);
	 				pushData(deptNameObj,targetDeptName,nodes[i].deptName);
	 				if(defaultField!=targetUserId&&defaultField!=targetUserName){
	 					pushData(def_fieldObj,defaultField,nodes[i].useraddress); 
	 				}
	 			} 
	 			rebackData(targetUserId,userIdObj);
	 			rebackData(targetUserNo,userNoObj);
	 			rebackData(targetUserName,userNameObj);
	 			rebackData(targetDeptId,deptIdObj);
	 			rebackData(targetDeptName,deptNameObj);
	 			if(defaultField!=targetUserId&&defaultField!=targetUserName){
	 			...
	 			//回调父页面方法
				try{
					parent.selectMultiAddress(defaultField,treeNode);
				}catch(e){} 
