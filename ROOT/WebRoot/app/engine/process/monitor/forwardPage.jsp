			     ...
			     $.post("processRuntimeExecuteForward.action",obj,function(data){
						...
						if(arr!=null){
							...
							if(arr=="success"){
								...
								art.dialog.tips("系统执行转发操作时不允许转发给本人，操作失败(错误号:ERROR-0002)",2);
							}else if(arr=="ERROR-10003"){
								art.dialog.tips("发送失败，请联系管理员(错误号:ERROR-10003)",5);
							}else{
								art.dialog.tips("转发失败，返回值异常(错误号:ERROR-10004)",5);
							}
						}else{
							art.dialog.tips("转发失败，返回值异常(错误号:ERROR-10004)",2);
						}
			     });
       }
       //调用父页面脚本，关闭办理窗口
...
<s:form name="ifromMain" id="ifromMain" method="post" action="processRuntimeExecuteForward" theme="simple">
		<table width="100%"  border="0">
			...
			<div style="padding:5px;padding-left:10px">已选择的转发人:</div>
