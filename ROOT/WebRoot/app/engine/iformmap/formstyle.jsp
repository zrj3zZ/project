	...
	<script type="text/javascript">
  	...
  	/*一些函数*/   
		//用于格式化 	 
		 ...
		 function savestyle(){
		 	var formcss = $('#formCss').val();
		 	if(formcss.lenght>2000){
		 	...
		 	$('#editForm').attr('action','sysEngineIForm_formcss_save.action');
		    var options = {
					error:errorFunc,
					success:successFunc 
				   };
...
<div region="north" border="false" style="height:40px;">
	<div  class="tools_nav">
							<a href="javascript:savestyle();" class="easyui-linkbutton" plain="true" iconCls="icon-save">保存</a>
...
<s:form  id="editForm" name="editForm" action="sysEngineIForm_formcss_save.action"  theme="simple">
				            	<s:if test="formCss!=null">
						 			<s:textarea   cssStyle="width:795px;height:480px;overflow-y:visible;color:#0000FF;background-image:url(iwork_img/engine/tips_css.png);background-position:center;background-repeat:no-repeat;"  id="formCss" name="formCss">
				            		</s:textarea>
						 		</s:if>
						 		<s:else>
				            		<s:textarea  value=".diy_class{

}" cssStyle="width:795px;height:480px;overflow-y:visible;color:#0000FF;background-image:url(iwork_img/engine/tips_css.png);background-position:center;background-repeat:no-repeat;"  id="formCss" name="formCss">
				            		</s:textarea>
				            		</s:else>
