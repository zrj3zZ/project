...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>
	...
	$(document).ready(function(){
	    $('#condition_grid').datagrid({
				...
				var url = 'url:ireport_designer_condition_edit.action?id='+rowData.ID+'&reportId='+reportId;
				openDialog('编辑',url);
				}
			});   
	});
	function edit(id,reportId){
	     var url = 'url:ireport_designer_condition_edit.action?id='+id+'&reportId='+reportId;
		 openDialog('编辑',url);
	}
	//添加和编辑窗口
	...
	function add(){
		...
		var url = 'url:ireport_designer_condition_add.action?showtype=input&reportId='+reportId+'&reportType='+reportType;
		openDialog('添加',url);
	}
	function del(){
	        var ids = new Array();
			...
			if(ids.length==0){
		      ...
		      var url='ireport_designer_condition_del.action';
	...
	function moveUp(){
	    ...
	    var url='ireport_designer_condition_moveUp.action';
	...
	function moveDown(){
	    ...
	    var url='ireport_designer_condition_moveDown.action';
	    $.post(url,{id:rows[0].ID,reportId:reportId},function(data){
	         if(data=='ok'){
	             $('#condition_grid').datagrid('reload');
	         }
	...
	function moveTop(){
	    ...
	    var url='ireport_designer_condition_moveTop.action';
	    $.post(url,{id:rows[0].ID,reportId:reportId},function(data){
	         if(data=='ok'){
	            $('#condition_grid').datagrid('reload');
	         }
	...
	function moveBottom(){
	    ...
	    var url='ireport_designer_condition_moveBottom.action';
	    $.post(url,{id:rows[0].ID,reportId:reportId},function(data){
	         if(data=='ok'){
	             $('#condition_grid').datagrid('reload');
	         }
  ...
  <body class="easyui-layout">
     <div region="center" style="padding:3px;border-top:0px;">					
					...
					<div>
						<table id="condition_grid" style="height:460px" iconCls="icon-edit" singleSelect="false" idField="ID" url="ireport_designer_condition_table.action?reportId=<s:property value='reportId'  escapeHtml='false'/>" >
									<thead>
										<tr>
										    <th field="CK" checkbox=true></th>
											<th field="FIELD_NAME" width="200" align="left">字段</th>
											<th field="FIELD_TITLE" width="80" >名称</th>	
											<th field="FIELD_DISPLAY" width="100" align="left" >外观</th>
											<th field="COMPARISON" width="50" align="left" >比较</th>											
											<th field="FIELD_DEF" width="100" align="left" >默认值</th>
