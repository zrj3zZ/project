...
draw2d.Task.prototype.onDoubleClick=function(){
	...
	var task = data.task;
	var tid = task.getId();
	var nodeTitle = task.getContent();
	art.dialog.prompt('请输入节点名称',function(val){
...
draw2d.Task.prototype.getContextMenu=function(){
	if(this.workflow.disabled)return null;
	var menu =new draw2d.ContextMenu(200, 330);
	var data = {task:this};
	menu.appendMenuItem(new draw2d.ContextMenuItem("任务设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'BaseMap'); 
				
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("路由设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'Route'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("表单设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("人工跳转设置", "properties-icon",data,function(x,y)
	{
		var data = this.getData();
		var task = data.task;
		var tid = task.getId();
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("规则跳转设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("线下任务", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				openDialog(tid,'OffLine'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("督办设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'Supervise'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("脚本触发器设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'jsTrigger'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("触发器设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'trigger'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("自定义按钮设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'diybtn'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("摘要设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
				openDialog(tid,'Summary'); 
			}));
	menu.appendMenuItem(new draw2d.ContextMenuItem("帮助设置", "properties-icon",data,function(x,y)
			{
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
...
draw2d.UserTask.prototype.getExtensionElementsXML=function(){
	...
	xml=xml+'<activiti:taskListener event="create" class="com.iwork.process.management.listener.RuntimeAssigneeListener"/>';
