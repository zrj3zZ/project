function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			imageName = "<font color='red'>文件大小超限</font>";
			break;
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
...
function fileQueued(file){
	addReadyFileInfo(file.id,file.name,file.index+1,"",file.size);
}
...
function addReadyFileInfo(fileid,fileName,message,status,size){
	//用表格显示
	...
	if(size!=null&&size!=""){
		col2.innerHTML = getNiceFileSize(size);
	}else{
		col2.innerHTML = "0kb";
	}
	...
	col4.innerHTML = "<a href='javascript:deleteFile(\""+fileid+"\")'><img src='../iwork_img/del3.gif' border='0'></a>";
	col1.style.width="200";
	col2.style.width="50";
	col3.style.width="50";
	col4.style.width="20";
}
