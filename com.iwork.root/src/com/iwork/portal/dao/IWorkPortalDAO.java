...
public class IWorkPortalDAO extends HibernateDaoSupport{
	
	public IWorkPortalDAO(){}
	...
	public List<IworkPortalUserItem> getUserItemAllList(String userid ,Long groupindex){
		String sql = "From IworkPortalUserItem where userid= ? and groupindex = ? order by orderIndex";
		DBUtilInjection d=new DBUtilInjection();
	    List lis=new ArrayList();
	    if ((userid != null) && (!"".equals(userid))) {
        	if(d.HasInjectionData(userid)){
		...
		Object[] values = {userid,groupindex};
		return this.getHibernateTemplate().find(sql,values);   
	}
