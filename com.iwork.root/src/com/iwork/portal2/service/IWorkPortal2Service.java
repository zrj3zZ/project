package com.iwork.portal2.service;
...
import com.iwork.portal2.dao.IWorkPortal2DAO;

public class IWorkPortal2Service {
	private IWorkPortal2DAO iworkPortal2DAO;
	private int count = 0;
	...
	public String getColumnHTML(){
		...
		String userid = UserContextUtil.getInstance().getCurrentUserId();
		 List<IworkPortalUserItem> list = iworkPortal2DAO.getUserItemAllList(userid);
		 ...
		 for(IworkPortalUserItem model:list){
			 IworkPortalOverall overallModel =  iworkPortal2DAO.getOverAllModel(model.getPortletId());
			 ...
			 if(groupIndex==null){
				 ...
				 iworkPortal2DAO.updatePortlet(model);
			 }
			 //获得栏目样式
	...
	public boolean movePortlet(Long groupId,Long portletid,String orderlist){
		...
		//加载移动的portlet
		IworkPortalUserItem model = iworkPortal2DAO.getPortlet(portletid);
		String userid = UserContextUtil.getInstance().getCurrentUserId();
		List<IworkPortalUserItem> groupList = iworkPortal2DAO.getUserItemAllList(userid,groupId);
		...
		if(model.getGroupindex().equals(groupId)){
			...
			for(int i=0;i<orderItemList.length;i++){ 
				IworkPortalUserItem item = iworkPortal2DAO.getPortlet(Long.parseLong(orderItemList[i].trim()));
				item.setOrderIndex(new Long(idlist[i]));
				iworkPortal2DAO.updatePortlet(item); 
			}
		}else{
	...
	public boolean removePortlet(Long id){
		boolean flag = false;
		if(id!=null){
			IworkPortalUserItem model = iworkPortal2DAO.getPortlet(id);
			if(model!=null){
				iworkPortal2DAO.removePortlet(model);
				flag = true;
			}
		}
		return flag;
	...
	public boolean setPortletList(String ids){
		...
		if(ids!=null){
			...
			if(idlist!=null&&idlist.length>0){
				//移除之前的门户设置
				String userid = UserContextUtil.getInstance().getCurrentUserId();
				//iworkPortal2DAO.removeUserPortletList(userid); 
				for(String id:idlist){
					if("".equals(id.trim()))continue;
					IworkPortalOverall ipoa = iworkPortal2DAO.getOverAllModel(Long.parseLong(id));
					...
					iworkPortal2DAO.savePortlet(model); 
				} 
				flag = true;
			}
		}
	...
	public String showOverAllTreeJSON(){
		...
		String userid = UserContextUtil.getInstance().getCurrentUserId();
		 List<IworkPortalUserItem> selectlist = iworkPortal2DAO.getUserItemAllList(userid);
		int count = 0;
	...
	private String getSubOverAllJson(String userid,String groupname,List<IworkPortalUserItem> selectlist){
		StringBuffer html = new StringBuffer();
		List<Map<String,Object>> rows = new ArrayList<Map<String,Object>>();
		List<IworkPortalOverall> list = iworkPortal2DAO.getOverAllList(groupname);
	...
	public boolean saveOverAllModel(IworkPortalOverall  model){
		...
		if(model!=null){
			if(model.getId()==null){
				iworkPortal2DAO.saveOverAll(model);
			}else{
				iworkPortal2DAO.updateOverAll(model);
			}
		}
		return flag;
	}
	...
	public boolean removeOverAllModel(Long  id){
		...
		//判断是否允许删除此栏目
		if(flag){
			IworkPortalOverall model = iworkPortal2DAO.getOverAllModel(id);
			iworkPortal2DAO.removeOverAll(model);
		}
		return flag;
	}

	public IWorkPortal2DAO getIworkPortal2DAO() {
		return iworkPortal2DAO;
	}

	public void setIworkPortal2DAO(IWorkPortal2DAO iworkPortal2DAO) {
		this.iworkPortal2DAO = iworkPortal2DAO;
	}
