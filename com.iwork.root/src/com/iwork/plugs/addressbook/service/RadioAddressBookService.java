...
import com.iwork.process.tools.EntrustAPI;


public class RadioAddressBookService {
	 ...
	 * @return 
	 */
	public String getRealJson(String actDefId){
		StringBuffer jsonHtml = new StringBuffer();
		List<Map<String,Object>> items = new ArrayList<Map<String,Object>>();
		// 查询委托列表
		List<OrgUser> entrustList = EntrustAPI.getInstance().getEntrustCreateList(actDefId, UserContextUtil.getInstance().getCurrentUserId());
		// 根据userid获取用户基本信息
		OrgUser orgUser = orgUserDAO.getUserModel(UserContextUtil.getInstance().getCurrentUserId());
		items.add(buildTreeUserNode(orgUser,true));
		for(OrgUser model:entrustList){
			if(model.getUsertype()!=null&&model.getUsertype().equals(new Long(0))){   //只显示组织用户，系统用户不显示 
				Map<String, Object> item = buildTreeUserNode(model,true);
				if(item!=null){
					items.add(item);
				}
			}
		...
		if(deptIds!=null&&deptIds.size()>0){
			if(deptIds.size()==1){
				for(Long deptId:deptIds){
					OrgDepartment od = orgDepartmentDAO.getBoData(deptId);
					//获得部门列表
...
//						if(model.getUsertype()!=null&&model.getUsertype().equals(new Long(0))){   //只显示组织用户，系统用户不显示 
					 if(model.getUsertype()!=null){ 
							Map<String, Object> item = buildTreeUserNode(model,false);
							if(item!=null){
								list.add(item);
...
//								}else if(model.getUsertype()!=null&&model.getUsertype().equals(new Long(0))){   //只显示组织用户，系统用户不显示
				...
				for(Long deptId:deptIds){
					OrgDepartment od = orgDepartmentDAO.getBoData(deptId);
					//获得部门列表
						Map<String,Object> childrenItem = buildTreeDeptNode(od); 
						if(childrenItem!=null){
							list.add(childrenItem);
						}
					}
...
//					}else if(model.getUsertype()!=null&&model.getUsertype().equals(new Long(0))){   //只显示组织用户，系统用户不显示
	 ...
	 * @return
	 */
	private Map<String, Object> buildTreeUserNode(OrgUser model,boolean flag){
		Map<String, Object> item = new HashMap<String, Object>();
		item.put("id", model.getUserid()); 
	 ...
	 * @return
	 */
	public String showRadioAddressList(String filedName,String searchKey){
...
//		html.append("<ul data-role=\"listview\">");
		...
		for(OrgUser model:list){
			html.append("<li style='line-height:30px;font-size:14px;'>\n");  
			html.append("<a href=\"javascript:setAddress('").append(filedName).append("','").append(model.getUserid()).append("[").append(model.getUsername()).append("]') \">").append(model.getUsername()).append("</a>\n");
			html.append("</li> \n");
			js.append("");
		} 
//		html.append("</ul>");
	 ...
	 * @return
	 */
	public String searchOrgJson(String searchKey,List<Long> deptIds){
		StringBuffer jsonHtml = new StringBuffer();	
		List<Map<String, Object>> list = new ArrayList();
