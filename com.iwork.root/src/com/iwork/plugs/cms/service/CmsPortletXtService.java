 ...
 *
 */
public class CmsPortletXtService {
...
//				}
				...
				rows.put("operate","<a href='#' title='设置' onclick=\"Set();\"><img src=iwork_img/btn_rwfp.gif border='0'></a> <a target='_blank' href='cmsInfoXtAction!index.action?portletid="+model.getId()+"' title='内容管理' ><img src=iwork_img/but_edit.gif border='0'></a>");
				}else{
                rows.put("operate","<a href='#' title='设置' onclick=\"Set();\"><img src=iwork_img/btn_rwfp.gif border='0'></a>");  
				}
				item.add(rows);			    
			}	  		
	...
	public String getKeyVal(String key,String type,String portletid){
		...
		for(int i=0;i<keyList.size();i++){
			...
			if(model.toString().equals(key)){
				msg="&nbsp;<span style='color:red'>此键值已存在！</span>";
			}
		}
