...
import com.ibpmsoft.project.zqb.dao.XSDZqbProjectManageDAO;
...
public class XSDZqbProjectManageService {
	private static Logger logger = Logger.getLogger(XSDZqbProjectManageService.class);
	...
	private XSDZqbProjectManageDAO xsdZqbProjectManageDAO;
	
	public void setXsdZqbProjectManageDAO(
			XSDZqbProjectManageDAO xsdZqbProjectManageDAO) {
		this.xsdZqbProjectManageDAO = xsdZqbProjectManageDAO;
	}
	...
	public List<UploadDocModel> showProjectDocList(String searchkey) {
		...
		return xsdZqbProjectManageDAO.getDocList(searchkey, owner);
	}
	...
	public String[] getProjectTypeBarChart3(boolean isSuperman) {
		...
		List<HashMap> templist = DemAPI.getInstance().getList(ProjectUUID,
				null, null);
		List<String> userlist = xsdZqbProjectManageDAO.getProjectGroupList();
		HashMap infodata = new HashMap();
		for (String userid : userlist) {
			List l = xsdZqbProjectManageDAO.getUserJs(userid);
	...
	private List<HashMap> getGroupList(String processNo) {
		HashMap conditionMap = new HashMap();
		return DemAPI.getInstance().getList(ProjectGroupUUID, conditionMap,"ID");
	}
	...
	private List<HashMap> getTaskMList(String groupid) {
		return xsdZqbProjectManageDAO.getTaskMList(groupid);
	}
	...
	public List<HashMap> getSearchProjectList(boolean superman, String type,
		...
		if (type.equals("SCORE")) {
			String scoreType = searchkey.substring(0, searchkey.indexOf(","));
			...
			if (scoreType.equals("关闭项目")) {
				for (HashMap hash : tmplist) {
					...
					if(hash.get("TYPENO")!=null&&hash.get("TYPENO").toString().equals("1")){
			...
			if (scoreType.equals("超期项目")) {
				for (HashMap hash : tmplist) {
					...
					if (xsdZqbProjectManageDAO.isProjectCQXM(hash.get("PROJECTNO")
							.toString())) {
						list.add(hash);
					}
				}
			...
			if (scoreType.equals("正常项目")) {
				for (HashMap hash : tmplist) {
					...
					if (xsdZqbProjectManageDAO.isProjectZCXM(hash.get("PROJECTNO")
							.toString())) {
						list.add(hash);
					}
				}
	...
	public boolean checkIsExists(HashMap hash) {
		boolean flag = false;
		flag = xsdZqbProjectManageDAO.getNumber(hash);
		return flag;
	}
	...
	public List<HashMap> getCloseProjectList(boolean superman, int pageNow,
		...
		List<HashMap> tmplist = xsdZqbProjectManageDAO.getCloseProjectList(
	...
	public List<HashMap> getFinishProjectList(boolean superman, int pageNow,
		...
		if(ismanager==1){
			if(cyrName!=null&&!cyrName.equals("")){
				parameter = xsdZqbProjectManageDAO.getCyrUserMap(map, cyrName);
			}else{
		...
		return xsdZqbProjectManageDAO.getHibernateTemplate().executeFind(
				new HibernateCallback() {
					public Object doInHibernate(org.hibernate.Session session)
						...
						for (Object[] object : list) {
							...
							String pjr = object[23]==null?"":object[23].toString();
	...
	public boolean projectFinish(String projectNo, Long instanceId) {
		...
		params.put(1, model.getDepartmentname());
		String id=DBUTilNew.getDataStr("ID","select * from ORGDEPARTMENT where DEPARTMENTNAME= ?　", params);
	...
	public String getProjectTypePieChart(boolean isSuperman, String type) {
		...
		if (isSuperman) {
			list = xsdZqbProjectManageDAO.getProjectTypeGroup(null);
		} else {
			list = xsdZqbProjectManageDAO.getProjectTypeGroup(UserContextUtil
	...
	public String getProjectTypePieChart2(boolean isSuperman, String type) {
		...
		List<ArrayList> list = null;
		if (isSuperman) {
			list = xsdZqbProjectManageDAO.getProjectTypeStatus(null);
		} else {
			list = xsdZqbProjectManageDAO.getProjectTypeStatus(UserContextUtil
	...
	public String getProjectTypePieChart4(boolean isSuperman, String type) {
		...
		List<ArrayList> list = null;
		list = xsdZqbProjectManageDAO.getProjectFengXian(pjNoList, UserContextUtil
				.getInstance().getCurrentUserFullName());
		if (list != null) {
	...
	public String[] getProjectTypeBarChart(boolean isSuperman) {
		...
		hash = xsdZqbProjectManageDAO.getProjectStageGroup(pjNoList,
	...
	public String getProjectTypeBarGrid(boolean isSuperman) {
		...
		if (isSuperman) {
			list = xsdZqbProjectManageDAO.getProjectStageList((null));
		} else {
			list = xsdZqbProjectManageDAO.getProjectStageList(UserContextUtil
					.getInstance().getCurrentUserId());
		}
		if (list != null) {
			jsonHtml.append("<table>").append("\n");
	...
	public String getQuestionTalkList(String projectNo, Long taskid,
		...
		List<UploadDocModel> qDocList = xsdZqbProjectManageDAO.getQuestionDocList(
				projectNo, taskid);
		html.append("");
		if (list != null) {
				...
				if (id.equals(questionId.toString())) {
					...
					if (count == 0) {
						...
						for (HashMap retalk : reTalklist) {
							...
							List<UploadDocModel> cDocList = xsdZqbProjectManageDAO
	...
	public String[] getManagerBarData() {
		...
		StringBuffer label = new StringBuffer();
		StringBuffer value = new StringBuffer();
		List<HashMap> list = xsdZqbProjectManageDAO.getProjectManagerUserData();
	...
	public String[] getYSZKChartData() {
		...
		StringBuffer label = new StringBuffer();
		StringBuffer value = new StringBuffer();
		List<HashMap> list = xsdZqbProjectManageDAO.getSSZKData();
		List labellist = new ArrayList();
		List fulllist = new ArrayList();
		List datalist1 = new ArrayList();
		List datalist2 = new ArrayList();
	...
	public List getJieDuanList(int index, int htje, int wsje) {
		List<HashMap> list = xsdZqbProjectManageDAO.getYSZKData();
		if (list != null) {
			String projectNo = "";
			String projectName = "";
			int je = 0;
	...
	public List getChartProjectList(int index) {
		List<HashMap> slist = xsdZqbProjectManageDAO.getProjectManagerUserData();
		if (slist != null) {
			String manager = "";
			String projectName = "";
	...
	public List getBar() {
		...
		for (HashMap map : tmplist) {
			...
			String gxsj = DBUTilNew.getDataStr( "GXSJ","select TO_CHAR(GXSJ,'YYYY-MM-DD HH24:MI') AS GXSJ from BD_PM_TASK where GXSJ=(select max(GXSJ) from BD_PM_TASK where projectno= ? )", params);
			map.put("GXSJ", gxsj);
			if (map.get("STATUS").equals("已完成") && map.get("XMJD") != null
					&&map.get("TYPENO")!=null&&map.get("TYPENO").toString().equals("1")) {
				continue;
			}
	...
	public int getTotalNum(boolean superman) {
		...
		int n = 0;
		for (HashMap data : tmplist) {
			if (data.get("TYPENO").toString().equals("1"))
				continue;
			if (!superman) {
				boolean flag = this.checkProjectSecurity(data, uc);
				if (!flag)
	...
	public List<HashMap> getRunProjectList1(boolean superman, int pageNumber,
		...
		Long orgRoleId = userModel.getOrgroleid();
		if (superman) {
			tmplist = xsdZqbProjectManageDAO.getRunProjectList1(pageNumber,pageSize, projectName, xmjd, startDate, customername,dgzt,sssyb,cyrName);
		} else {
			if(orgRoleId==11){
				tmplist = xsdZqbProjectManageDAO.getRunProjectList3(owner, pageNumber,
						pageSize, projectName, xmjd, startDate, customername,dgzt);
			}else{
				Long ismanager = getIsManager(uc);
				tmplist = xsdZqbProjectManageDAO.getRunProjectList2(owner, pageNumber,
	...
	public List<HashMap> getRunProjectList3(boolean superman, int pageNumber,
		...
		List<HashMap> list = new ArrayList();
		List<HashMap> tmplist = new ArrayList<HashMap>();
		tmplist = xsdZqbProjectManageDAO.getRunProjectList1(pageNumber,
		...
		for (HashMap data : tmplist) {
			...
			Map params = new HashMap();
			params.put(1,data.get("PROJECTNO"));
			String gxsj = DBUTilNew.getDataStr("GXSJ","select TO_CHAR(GXSJ,'YYYY-MM-DD HH24:MI') AS GXSJ from BD_PM_TASK where GXSJ=(select max(GXSJ) from BD_PM_TASK where projectno= ? )", params);
			data.put("GXSJ", gxsj);
			data.put("ISYD",
					!sd.getSyMsgSize(data.get("INSTANCEID").toString()));
	...
	public List getFinishProjectListSize1(boolean superman, String projectName,
		...
		List params = new ArrayList();
		StringBuffer sb = new StringBuffer("SELECT COUNT(*) CNUM FROM (SELECT DISTINCT J.*,D.NUM FROM (SELECT * FROM (SELECT C.*,ROWNUM RN from (SELECT A.ID,A.PROJECTNO,CUSTOMERNAME,OWNER,NVL(A.HTJE, 0) HTJE,(NVL(A.HTJE, 0) - NVL(E.SSJE, 0)) WSJE,A.XMJD,E.GROUPID,E.SSJE,F.jdmc,TASK_NAME,E.SPZT, E.MANAGER, E.PJINSID, E.XGWTINSID,BINDTABLE.INSTANCEID,nvl(g.ldpj,'评价') ldpj,g.pjr pjr FROM (SELECT DISTINCT J.* FROM BD_ZQB_PJ_BASE J ");
		if (!superman) {
			if(ismanager==1){
				sb.append("INNER JOIN (select projectno from bd_zqb_pj_base a where SUBSTR(owner,0, instr(owner,'[',1)-1) in (");
				String[] owners = parameter.get("SUBSTR(owner,0, instr(owner,'[',1)-1)").split(",");
	...
	public List getCloseProjectListSize1(boolean superman, String projectName,
		...
		if (!flag) {
			if(ismanager==1){
				...
				sb.append(" INNER JOIN ("
						// 项目的负责人、现场负责人
						+ " select projectno from bd_zqb_pj_base a where SUBSTR(owner,0, instr(owner,'[',1)-1) =? or SUBSTR(manager,0, instr(manager,'[',1)-1) = ? or createuserid = ? union all"
						// 任务阶段负责人
						...
						+ " left join BD_ZQB_GROUP b on a.dataid = b.id) d on c.instanceid = d.instanceid where userid = ? union all");
				params.add(owner);
				params.add(owner);
				params.add(username);
				params.add(owner);
		...
		sb.append(" ) F ON E.GROUPID = F.ID LEFT JOIN (SELECT * FROM SYS_ENGINE_FORM_BIND WHERE formid = 91 and metadataid = 101 and INSTANCEID is not null) BINDTABLE ON A.id = BINDTABLE.dataid LEFT JOIN (select * from BD_ZQB_XMRWPJB a inner join (select max(id) id from BD_ZQB_XMRWPJB group by groupid,projectno) b on a.id=b.id) G ON A.PROJECTNO = G.PROJECTNO AND E.GROUPID = G.GROUPID WHERE A.STATUS ='已完成' AND A.TYPENO is null ORDER BY A.ID DESC) C) J INNER JOIN (SELECT PROJECTNO,CUSTOMERNAME, COUNT(*) NUM FROM (SELECT * FROM (SELECT A.ID,A.PROJECTNO,CUSTOMERNAME,OWNER,NVL(A.HTJE, 0) HTJE,(NVL(A.HTJE, 0) - NVL(E.SSJE, 0)) WSJE,A.XMJD,E.GROUPID,E.SSJE,F.jdmc,TASK_NAME,E.SPZT, E.MANAGER, E.PJINSID, E.XGWTINSID,BINDTABLE.INSTANCEID,ROWNUM RN,nvl(g.ldpj,'评价') ldpj FROM  (SELECT DISTINCT J.* FROM BD_ZQB_PJ_BASE J ");
		if (!flag) {
			if(ismanager==1){
				sb.append(" INNER JOIN (select projectno from bd_zqb_pj_base a where SUBSTR(owner,0, instr(owner,'[',1)-1) in (");
				String[] owners = parameter.get("SUBSTR(owner,0, instr(owner,'[',1)-1)").split(",");
	...
	public List<HashMap> getYxmcyrz(boolean superman, int pageNumberXM,
		...
		if (!superman) {
			if(ismanager==1){
				...
				sql.append("select * from (select projectname, instanceid, xmjd, projectno, owner name from BD_ZQB_PJ_BASE BOTABLE inner join SYS_ENGINE_FORM_BIND BINDTABLE on BOTABLE.id = BINDTABLE.dataid and BINDTABLE.INSTANCEID is not null and BINDTABLE.formid = 91 and BINDTABLE.metadataid = 101 and xmjd<>'持续督导' and status = '执行中' and SUBSTR(owner,0, instr(owner,'[',1)-1)=? union select projectname, instanceid, xmjd, projectno, manager name from BD_ZQB_PJ_BASE BOTABLE inner join SYS_ENGINE_FORM_BIND BINDTABLE on BOTABLE.id = BINDTABLE.dataid and BINDTABLE.INSTANCEID is not null and BINDTABLE.formid = 91 and BINDTABLE.metadataid = 101 and xmjd<>'持续督导' and status = '执行中'  and SUBSTR(manager,0, instr(manager,'[',1)-1)=? union select projectname, instanceid, xmjd, projectno,createuserid||'['||createuser||']' name from BD_ZQB_PJ_BASE BOTABLE  inner join SYS_ENGINE_FORM_BIND BINDTABLE on BOTABLE.id = BINDTABLE.dataid and BINDTABLE.INSTANCEID is not null and BINDTABLE.formid = 91 and BINDTABLE.metadataid = 101 and xmjd<>'持续督导' and status = '执行中' and createuserid=? union select b.projectname, b.instanceid, b.xmjd, b.projectno,a.name from (select b.instanceid,a.userid||'['||a.name||']' name from BD_ZQB_GROUP a inner join SYS_ENGINE_FORM_BIND b  on formid = (select subformid from sys_engine_subform t where subtablekey = 'SUBFORM_XMCYLB' and rownum = 1) and metadataid = '105' and a.id = b.dataid and b.instanceid is not null and name=?) a inner join (select a.projectno,a.xmjd,a.projectname,b.instanceid from BD_ZQB_PJ_BASE a inner join SYS_ENGINE_FORM_BIND b on formid = '91' and a.status='执行中' and metadataid = '101' and a.id = b.dataid and b.instanceid is not null)  b on a.instanceid=b.instanceid) a order by name");
				params.add(owner);
				params.add(owner);
				params.add(owner);
				params.add(username);
		...
		tmplist = xsdZqbProjectManageDAO.getListCYR(startRow, pageSize, sql.toString(),params);
	...
	public List getXMListSize(boolean superman) {
		...
		tmplist = xsdZqbProjectManageDAO.getListSize(sql.toString(),params);
		return tmplist;
	}
	public List<Map<String, Object>> getXMList(boolean superman,
		...
		tmplist = xsdZqbProjectManageDAO.getList(startRow, pageSize, sql.toString(), params);
		HashMap<String, Object> map = null;
		if (tmplist.size() > 0) {
			for (HashMap data : tmplist) {
	...
	public List<Map<String, Object>> getXMExpList(boolean superman,
		...
		List params = new ArrayList();
		if (!superman) {
			sql.append("select projectname,instanceid,xmjd,projectno,owner,manager from BD_ZQB_PJ_BASE BOTABLE  inner join SYS_ENGINE_FORM_BIND BINDTABLE on BOTABLE.id = BINDTABLE.dataid and BINDTABLE.INSTANCEID is not null   and BINDTABLE.formid = 91 and BINDTABLE.metadataid = 101 and  xmjd<>'持续督导' and status='执行中'  and (SUBSTR(owner,0, instr(owner,'[',1)-1)=? or SUBSTR(manager,0, instr(manager,'[',1)-1)=? ) union select b.projectname, b.instanceid, b.xmjd, b.projectno, b.owner,b.manager from (select b.instanceid, a.name from BD_ZQB_GROUP a inner join SYS_ENGINE_FORM_BIND b on formid = (select subformid from sys_engine_subform t where subtablekey = 'SUBFORM_XMCYLB' and rownum = 1) and metadataid = '105' and a.id = b.dataid and b.instanceid is not null and a.name=? ) a inner join (select a.projectno, a.xmjd, a.projectname, b.instanceid,a.owner,a.manager from BD_ZQB_PJ_BASE a inner join SYS_ENGINE_FORM_BIND b on formid = '91' and a.status='执行中' and metadataid = '101' and a.id = b.dataid and b.instanceid is not null) b on a.instanceid = b.instanceid ");
			params.add(owner);
			params.add(owner);
			params.add(uc._userModel.getUsername());
		} else {
			sql.append("select projectname,instanceid,xmjd,projectno,owner,manager from BD_ZQB_PJ_BASE BOTABLE  inner join SYS_ENGINE_FORM_BIND BINDTABLE on BOTABLE.id = BINDTABLE.dataid and BINDTABLE.INSTANCEID is not null and BINDTABLE.formid = 91 and BINDTABLE.metadataid = 101  and xmjd<>'持续督导' and status='执行中' ");
		}
		tmplist = xsdZqbProjectManageDAO.getList(startRow, pageSize, sql.toString(),params);
	...
	public int getTotalCYNum(boolean superman) {
		...
		if (!superman) {
			if(ismanager==1){
				...
				parameter.get("createuserid").split(",");
				...
				while (rs.next()) {
					...
					if (listJD.indexOf(jdbh) < 0) {
						...
						content.append("<td role=\"gridcell\"  rowspan=\""
								...
								+ ssje
								+ "\" aria-describedby=\"subformSUBFORM_SXCYRY_NAME\">"
								+ ssje + "</td>  " + "                 ");
					}
	...
	public String loadProjectNew(String projectno) {
		...
		ResultSet rsZL = null;
		try {
			rsZL = stmt.executeQuery(sqlZL.toString());
			stmt =conn.prepareStatement(sqlZL.toString());
			stmt.setString(1, projectno);
			rsZL = stmt.executeQuery();
			int i = 0;
	...
	public String loadXgwt(String projectNo) {
		...
		HashMap conditionMap = new HashMap();
		conditionMap.put("PROJECTNO", projectNo);
		List<HashMap> list = DemAPI.getInstance().getList("90c55ee0bc9244f99f546932e50d3052", conditionMap, "ID DESC");
		StringBuffer content = new StringBuffer();
		content.append("										<table class=\"ui-jqgrid-htable ke-zeroborder\" role=\"grid\""
				+ " aria- labelledby=\"gbox_subformSUBFORM_BGSRZXX\"           "
				+ " style=\"width:850px;\" border=\"0\" cellpadding=\"0\"      "
	...
	public String checkRwid(String projectNo, String groupid) {
		String sql = "select MAX(id) id from bd_pm_group t where id<" + groupid;
		String id = "";
		...
		if (lastId != 0) {
			...
			if (list.size() > 0) {
				...
				if (list1.size() > 0) {// 去大于1的
					sql = "select min(id) id from bd_pm_group t where id>" + groupid;
					int nextId = DBUtil.getInt(sql, "ID");// 上一任务id
					if (nextId != 0) {
		...
		String name = DBUtil.getString("select FILE_SRC_NAME from sys_upload_file where file_id='" + sxzlmb + "'", "FILE_SRC_NAME");
	...
	public boolean deleteTask(Long instanceid) {
		...
		String groupid = hashMap.get("GROUPID") == null ? "" : hashMap.get(
				"GROUPID").toString();
		StringBuffer sql = new StringBuffer("delete from BD_XP_ZYDDSXLCB where task_name='" + task_name
	...
	public String getJdzl(String projectNo, String groupid, Long instanceId) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (HashMap zlmap : l) {
				...
				content.append("<div id=\""
						...
						+ "	<div style=\"align:right;float: right;\">		<a href=\"javascript:uploadifyReomve('JDZL"
						+ i
						+ "','"
						+ zl
						+ "','"
						+ zl
						+ "');\">"
						+ "<img src=\"/iwork_img/del3.gif\"></a>	</div>	<span><a href=\"uploadifyDownload.action?fileUUID="
						+ zl + "\" target=\"_blank\">"
		...
		String c = DBUtil.getString("select * from BD_ZQB_KM_INFO where id=" + groupid, "CONTENT");
	...
	public List<HashMap> getXMCYList(String userid) {
		List<HashMap> list = new ArrayList();
		list = xsdZqbProjectManageDAO.getXMCYList(userid);
		return list;
	}
	...
	public List getXgwthfx(int pageNumber, int pageSize, String projectName,
		...
		List<HashMap> l = DemAPI.getInstance().getList(
				"f5c75cf3d2714f7aa1b5d943ef1d76b0", conditionMap, "id desc",
				pageSize, startRow);
		for (HashMap map : l) {
			// 根据任务id获取任务名称，并将值放入map中
			if (map.get("TASKNO") != null && !map.get("TASKNO").equals("")) {
				String sql = "select JDMC from BD_ZQB_KM_INFO where id="
						+ map.get("TASKNO");
				String taskname = DBUtil.getString(sql, "JDMC") == null ? ""
						: DBUtil.getString(sql, "JDMC");
				map.put("TASKNAME", taskname);
			}
			String sql1 = "select instanceid from BD_ZQB_PJ_BASE BOTABLE inner join SYS_ENGINE_FORM_BIND BINDTABLE on BOTABLE.id = BINDTABLE.dataid and BINDTABLE.INSTANCEID is not null and BINDTABLE.formid = 91 and BINDTABLE.metadataid = 101 and projectno='" + map.get("XMBH") + "'";
			String xmid = DBUtil.getString(sql1, "instanceid") == null ? "" : DBUtil.getString(sql1, "instanceid");
			map.put("XMID", xmid);
		}
		return l;
	...
	public void expProjectList(HttpServletResponse response, String zcPro) {
		...
		String owner = uc._userModel.getUserid();// + "["
				//+ uc._userModel.getUsername() + "]";
		if (isSuperMan) {
			slist = xsdZqbProjectManageDAO.expProjectList1();
		} else {
			slist = xsdZqbProjectManageDAO.expProjectList2(owner);
		}
		// 第一步，创建一个webbook，对应一个Excel文件
		HSSFWorkbook wb = new HSSFWorkbook();
		// 第二步，在webbook中添加一个sheet,对应Excel文件中的sheet
		...
		int endRow=0;
		int beginSize=0;
		//JDMC=尽职调查, XMBZ=null, OWNER=DOUJIWEI[窦吉卫], KHLXDH=null, MEMO=请问, MANAGER=WEIWEI[魏蔚], STARTDATE=2015-12-18, FZJGLXR=null, SPZT=未提交, PROJECTNO=PM2015-12-1980, SSJE=0.0, ENDDATE=null, LCID=45416, ZCLR=null, GROUPID=24, PROJECTNAME=王瑞丰测试123, FILENAME=null, GGJZR=null, CUSTOMERNAME=王瑞丰测试123, FZJGMC=null, XMmanager=WEIWEI[魏蔚], ZCLRDH=null, SBJZR=null, WBCLRJG=null, HTJE=0.0, KHLXR=王瑞丰
		...
		if(Arrays.asList(split).contains("SSSYB")){
			cell1 = row1.createCell((short) z++);
			cell1.setCellValue("所属事业部");
			cell1.setCellStyle(style5);
		}
		
		if(Arrays.asList(split).contains("CLSLR")){
			cell1 = row1.createCell((short) z++);
			cell1.setCellValue("材料受理日");
			cell1.setCellStyle(style5);
		}
		
		if(Arrays.asList(split).contains("SHTGR")){
			cell1 = row1.createCell((short) z++);
			cell1.setCellValue("审核通过日");
			cell1.setCellStyle(style5);
		}
		
		if(Arrays.asList(split).contains("GZLXR")){
			cell1 = row1.createCell((short) z++);
			cell1.setCellValue("股转联系人");
			cell1.setCellStyle(style5);
		}
		
		if(Arrays.asList(split).contains("GZLXDH")){
			cell1 = row1.createCell((short) z++);
			cell1.setCellValue("股转联系电话");
			cell1.setCellStyle(style5);
		}
		
		if(Arrays.asList(split).contains("GZYJDZ")){
			cell1 = row1.createCell((short) z++);
			cell1.setCellValue("股转邮件地址");
			cell1.setCellStyle(style5);
		}
		
		if(Arrays.asList(split).contains("YJSBSJ")){
			cell1 = row1.createCell((short) z++);
		...
		for (HashMap hashMap : slist) {
			...
			if(hashMap.get("GROUPID")!=null&&!"".equals(hashMap.get("GROUPID").toString())){
				xgwtList=xsdZqbProjectManageDAO.expProjectXGWTList(hashMap.get("PROJECTNO").toString(),hashMap.get("GROUPID").toString());
				pjList=xsdZqbProjectManageDAO.expProjectPJList(hashMap.get("PROJECTNO").toString(),hashMap.get("GROUPID").toString());
			}
	...
	public List<HashMap> showDailyList(String projectNo, int pageNumber, int pageSize,String startdate,String enddate) {
		return xsdZqbProjectManageDAO.getShowDailyList(projectNo,pageNumber,pageSize,startdate,enddate);
	}
	...
	public List<HashMap> getDaily(String projectno){
		...
		String owner = uc._userModel.getUserid();// + "["
				//+ uc._userModel.getUsername() + "]";
		/*if (superman) {
			tmplist = xsdZqbProjectManageDAO.getDailyRunProjectList();
		} else {
			tmplist = xsdZqbProjectManageDAO.getDailyRunProjectList1(owner);
		}*/
		tmplist.add(projectno);
		List<HashMap> list = new ArrayList<HashMap>();
		list=xsdZqbProjectManageDAO.getDaily(tmplist);
		return list;
	}
	...
	public List<HashMap> getDailySize(String projectno){
		...
		tmplist.add(projectno);
		List<HashMap> list = new ArrayList<HashMap>();
		list=xsdZqbProjectManageDAO.getDailySize(tmplist);
		return list;
	}
	...
	public String getCXDD(String customerno) {
		List datalist=new ArrayList();
		StringBuffer jsonHtml = new StringBuffer();
		//KHFZR   FHSPR
		Map params = new HashMap();
		params.put(1,customerno);
		String khfzr = DBUTilNew.getDataStr("KHFZR","select KHFZR from BD_MDM_KHQXGLB WHERE khbh= ? ",params);
		String fhspr = DBUTilNew.getDataStr("FHSPR","select FHSPR from BD_MDM_KHQXGLB WHERE khbh= ? ", params);
		HashMap map = new HashMap();
		map.put("KHFZR", khfzr);
		map.put("FHSPR", fhspr);
		datalist.add(map);
	...
	public List<HashMap> showXYZL(String projectNo) {
		return xsdZqbProjectManageDAO.showXYZLList(projectNo);
	}
	public List<HashMap> getXMZLList(String projectNo,String xmzlListformid) {
		return xsdZqbProjectManageDAO.getXMZLList(projectNo,xmzlListformid);
	}
	public Integer deleteXYZL(String zlxxid) {
		return xsdZqbProjectManageDAO.deleteXYZLList(zlxxid);
	}
	public Integer delatexmzllist(String listid) {
		return xsdZqbProjectManageDAO.delatexmzllist(listid);
	}
	public Integer savaxmzllist(String chk_list,String projectNo) {
		return xsdZqbProjectManageDAO.savaxmzllist(chk_list,projectNo);
	}
	public Integer updateXYZL(String zlxxid) {
		return xsdZqbProjectManageDAO.updateXYZLList(zlxxid);
	}

	public String getRestNum(Long instanceId) {
		BigDecimal bd100=new BigDecimal("100.00");
		List<HashMap> CLR=ProcessAPI.getInstance().getFromSubData(instanceId, "SUBFORM_CLR");
		if(CLR==null){
		...
		List<HashMap> CLJG = ProcessAPI.getInstance().getFromSubData(instanceId, "SUBFORM_CLJG");
		if(CLJG==null){
