...
public class zqbLuyouJumpGGFiveEvent  extends SysJumpTriggerEvent{
	public zqbLuyouJumpGGFiveEvent(UserContext me, Task task) {
		super(me, task); 
	}
	/**
	 * 获得下一个办理节点
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isZhongYinDmGglc&&isExists("CWSCBFZR2")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR2",SystemConfig._ggsplcConf.getJd6())&&!isSkip(hashMap, "CWSCBFZR2",SystemConfig._ggsplcConf.getJd6()))){
				 ...
				 }else if(isZhongYinDmGglc&&isExists("FBBWJSHR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FBBWJSHR",SystemConfig._ggsplcConf.getJd10()))){
					 return SystemConfig._ggsplcConf.getJd10();
