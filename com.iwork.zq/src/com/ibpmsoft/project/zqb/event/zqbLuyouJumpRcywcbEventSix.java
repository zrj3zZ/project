 ...
 *
 */
public class zqbLuyouJumpRcywcbEventSix extends SysJumpTriggerEvent {
	public zqbLuyouJumpRcywcbEventSix(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isZhongYinDmGglc&&isExists("CWSCBFZR3")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR3",SystemConfig._rcywcbLcConf.getJd7())&&!isSkip(hashMap, "CWSCBFZR3",SystemConfig._rcywcbLcConf.getJd7()))){
	...
	public  List<UserContext> getNextUserList(){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 Long excutionId = this.getExcutionId();
				 String actdefid = this.getActDefId();
				 if(isZhongYinDmGglc&&isExists("CWSCBFZR3")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR3",nextStepId)&&!isSkip(hashMap, "CWSCBFZR3",nextStepId))){
