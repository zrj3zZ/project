 ...
 *
 */
public class zqbLuyouJumpRcywcbEventThree extends SysJumpTriggerEvent {
	public zqbLuyouJumpRcywcbEventThree(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("FHSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FHSPR",SystemConfig._rcywcbLcConf.getJd4())&&!isSkip(hashMap, "FHSPR",SystemConfig._rcywcbLcConf.getJd4()))){
