 ...
 * 
 */
public class ShanXiZqbLuyouJumpCwZlgdEvent extends SysJumpTriggerEvent {
	private static Logger logger = Logger.getLogger(ShanXiZqbLuyouJumpCwZlgdEvent.class);
	public ShanXiZqbLuyouJumpCwZlgdEvent(UserContext me, Task task) {
		super(me, task);
	}
	...
	public String getNextStepId(){
		...
		if(actStepId.equals(SystemConfig._cwZlgdSpLcConf.getJd1())){
			if(!spry.get("NODE1").toString().equals("")){
				return SystemConfig._cwZlgdSpLcConf.getJd2();
			}else if(!spry.get("NODE2").toString().equals("")){
				return SystemConfig._cwZlgdSpLcConf.getJd3();
			}else if(!spry.get("NODE3").toString().equals("")){
				return SystemConfig._cwZlgdSpLcConf.getJd4();
			}else{
				return SystemConfig._cwZlgdSpLcConf.getEnd();
			}
		}else if(actStepId.equals(SystemConfig._cwZlgdSpLcConf.getJd2())){
		...
		}else if(actStepId.equals(SystemConfig._cwZlgdSpLcConf.getJd3())){
	...
	public HashMap<String, String> getSpry(Long instanceId) {
		StringBuffer sql=new StringBuffer("SELECT (SELECT CWGWXX.OWNER FROM BD_ZQB_CWZLGD CWZLGD INNER JOIN BD_ZQB_CWGWXMXX CWGWXX ON CWZLGD.XMBH=CWGWXX.XMBH WHERE CWZLGD.LCBS = ?) AS NODE1,NODE1 AS NODE2,NODE2 AS NODE3,EXTENDSFSP,NODE1SFSP AS NODE2SFSP,NODE2SFSP AS NODE3SFSP FROM BD_ZQB_SXSPRYSZ WHERE LCMC='资料归档' AND XMLX='一般性财务顾问项目'");
