 ...
 *
 */
public class zqbDGDYAfterSaveEventone extends ProcessStepTriggerEvent {

	private static final String CN_FILENAME = "/common.properties"; //抓取网站配置文件     
	public zqbDGDYAfterSaveEventone(UserContext uc, HashMap hash) {
		super(uc, hash);
		...
		if(dataMap!=null){
			...
			if (target != null) {
    				...
    				if(email!=null&&!email.equals("") && uc != null){
    					...
    					MessageAPI.getInstance().sendSysMail(username, email, "工作底稿调阅", mailContent+"<br>"+noticename);
    				}
			}
		}
		return flag;
