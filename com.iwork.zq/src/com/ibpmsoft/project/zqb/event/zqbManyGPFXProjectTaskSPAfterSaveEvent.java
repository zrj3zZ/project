...
public class zqbManyGPFXProjectTaskSPAfterSaveEvent extends ProcessTriggerEvent {

	public zqbManyGPFXProjectTaskSPAfterSaveEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
			...
			List<HashMap> lcbsexists=DemAPI.getInstance().getList(PROJECT_TASK_UUID, conditionMap, null);
			 if(lcbsexists!=null&&lcbsexists.size()==1){
				for(HashMap<String,Object> hash:lcbsexists){
	    			...
	    			hash.put("SPZT", "审批通过");
	    			hash.put("STEPID",SystemConfig._gpfxXmlcConf.getEnd());
    			...
    			String actStepId=SystemConfig._gpfxXmlcConf.getEnd();
