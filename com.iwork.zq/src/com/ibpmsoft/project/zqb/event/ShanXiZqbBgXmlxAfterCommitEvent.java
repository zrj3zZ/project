 ...
 * 
 */
public class ShanXiZqbBgXmlxAfterCommitEvent extends ProcessStepTriggerEvent {
	
	private final static String CN_FILENAME = "/common.properties";
	private static Logger logger = Logger.getLogger(ShanXiZqbBgXmlxAfterCommitEvent.class);
	public ShanXiZqbBgXmlxAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(!dataMap2.isEmpty()){
				...
				content.append(jyf).append("已提交并购项目立项，请审核!");
				...
				if(SystemConfig._bgXmlxSpLcConf.getJd3().equals(actStepId)){
					deleteData(processActDefId,SystemConfig._bgXmlxSpLcConf.getJd3(),userid,instanceId);
				}
				...
				if (target != null) {
					if (!content.equals("")) {
						...
						if (email != null && !email.equals("") && ut != null) {
							...
							MessageAPI.getInstance().sendSysMail(senduser, email, "并购项目立项审核", content.toString());
						}
					}
				}
			}
	...
	public HashMap<String,Object> getDataMap(String xmbh,Long dqjdbh){
		...
		sql.append("SELECT BGCZ.ID,BGCZ.JYF,BGCZ.INSTANCEID,NVL(BGCZ.JDBH,0) JDBH,NVL(XMINFO.SORTID,0) SORTID,(SELECT SORTID FROM BD_ZQB_TYXM_INFO WHERE ID = ?) LCSORTID FROM BD_ZQB_BGXMLX BGLX INNER JOIN BD_ZQB_BGZZLXXX BGCZ ON BGLX.XMBH=BGCZ.XMBH LEFT JOIN BD_ZQB_TYXM_INFO XMINFO ON BGCZ.JDBH=XMINFO.ID WHERE BGLX.XMBH = ?");
