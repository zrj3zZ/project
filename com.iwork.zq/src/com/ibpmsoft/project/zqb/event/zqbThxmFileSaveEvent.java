 ...
 *
 */
public class zqbThxmFileSaveEvent extends ProcessTriggerEvent {

	public zqbThxmFileSaveEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			String sxmc = dataMap.get("XMMC")==null?"":dataMap.get("XMMC").toString();
				...
				if(lcbsexists!=null&&lcbsexists.size()==1){
					...
					for(HashMap<String,Object> hash:lcbsexists){
						...
						LogUtil.getInstance().addLog(dataId, "投行项目流程管理", "流程："+value+"---已审核完成");
					}
				}else{
					// 保存到投行项目数据表中
			...
			//编辑短信内容
			mailContent =dataMap.get("XMMC").toString()+"的流程，已审核通过！";
			//发送短信
			String assignee = dataMap.get("QCRID").toString();
			String username = "";
			...
			if (target != null) {
				if (!smsContent.equals("")) {
    				...
    				if(email!=null&&!email.equals("") && uc != null){
    					...
    					MessageAPI.getInstance().sendSysMail(username, email, "投行项目流程审核通过", mailContent+"<br>"+noticename);
    				}
				}
			}
		}
