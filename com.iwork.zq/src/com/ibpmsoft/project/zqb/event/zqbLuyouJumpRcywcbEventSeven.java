 ...
 *
 */
public class zqbLuyouJumpRcywcbEventSeven extends SysJumpTriggerEvent {
	public zqbLuyouJumpRcywcbEventSeven(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				...
				if(isExists("FBBWJSHR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FBBWJSHR",SystemConfig._rcywcbLcConf.getJd8()))){
	...
	public  List<UserContext> getNextUserList(){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 Long excutionId = this.getExcutionId();
				 String actdefid = this.getActDefId();
				 if(isExists("FBBWJSHR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FBBWJSHR",nextStepId))){
