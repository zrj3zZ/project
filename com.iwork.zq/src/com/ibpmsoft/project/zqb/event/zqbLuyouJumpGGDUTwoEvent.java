 ...
 *
 */
public class zqbLuyouJumpGGDUTwoEvent extends SysJumpTriggerEvent {
	public zqbLuyouJumpGGDUTwoEvent(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("FHSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FHSPR",SystemConfig._cxddfqlcConf.getJd3())&&!isSkip(hashMap, "FHSPR",SystemConfig._cxddfqlcConf.getJd3()))){
					 return SystemConfig._cxddfqlcConf.getJd3();
				 }else if(isExists("ZZSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "ZZSPR",SystemConfig._cxddfqlcConf.getJd4())&&!isSkip(hashMap, "ZZSPR",SystemConfig._cxddfqlcConf.getJd4()))){
	...
	public  List<UserContext> getNextUserList(){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("FHSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FHSPR",nextStepId)&&!isSkip(hashMap, "FHSPR",nextStepId))){
