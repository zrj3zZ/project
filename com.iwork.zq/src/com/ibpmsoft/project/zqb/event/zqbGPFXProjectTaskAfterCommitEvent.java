 ...
 * 
 */
public class zqbGPFXProjectTaskAfterCommitEvent extends ProcessStepTriggerEvent {

	public zqbGPFXProjectTaskAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if (dataMap != null) {
			...
			if (lcbsexists != null && lcbsexists.size() == 1) {
				for (HashMap<String, Object> hash : lcbsexists) {
					...
					flag = DemAPI.getInstance().updateFormData(
							...
							false);
					if(flag){
						PROJECT_UUID=DBUtil.getString("SELECT UUID FROM SYS_DEM_ENGINE WHERE TITLE = '股票发行项目'", "UUID");
						//select b.jdmc from (select max(groupid) groupid from BD_ZQB_GPFXXMRWB where projectno='PM2015-10-1549' and spzt='已提交') a left join BD_ZQB_TYXM_INFO b on a.groupid = b.id;
						...
						if(XMJD!=null&&!"".equals(XMJD)){
							...
							for (HashMap<String, Object> object : list) {
								...
								flag = DemAPI.getInstance().updateFormData(
										PROJECT_TASK_UUID,
										Long.parseLong(object.get("INSTANCEID").toString()),
										object, Long.parseLong(object.get("ID").toString()),
										false);
							}
						}
					}
			...
			if(flag){
				...
				if("1".equals(config)){
					if(!isOwner()&&SystemConfig._gpfxXmlcConf.getJd1().equals(actStepId)){
						HashMap map = new HashMap();
						map.put("PROJECTNO",projectno);
						List<HashMap> list2 = DemAPI.getInstance().getList(PROJECT_UUID, map, null);
						SendContent(list2,smsContent,"OWNER");
					}else if(isOwner()&&SystemConfig._gpfxXmlcConf.getJd1().equals(actStepId)){
						conditionMap.clear();
					...
					}else if (SystemConfig._gpfxXmlcConf.getJd2().equals(actStepId)){
						conditionMap.clear();
						List<HashMap> rwlist = DemAPI.getInstance().getList(demUUID, conditionMap, null);
						if(rwlist.get(0).get("CSFZR")!=null&&!"".equals(rwlist.get(0).get("CSFZR").toString())){
							SendContent(rwlist,smsContent,"CSFZR");
					...
					}else if (SystemConfig._gpfxXmlcConf.getJd3().equals(actStepId)){
						conditionMap.clear();
						List<HashMap> rwlist = DemAPI.getInstance().getList(demUUID, conditionMap, null);
					...
					}else if (SystemConfig._gpfxXmlcConf.getJd4().equals(actStepId)){
						conditionMap.clear();
						List<HashMap> rwlist = DemAPI.getInstance().getList(demUUID, conditionMap, null);
