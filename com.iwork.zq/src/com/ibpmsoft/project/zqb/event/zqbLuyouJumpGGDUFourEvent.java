...
public class zqbLuyouJumpGGDUFourEvent extends SysJumpTriggerEvent{
	public zqbLuyouJumpGGDUFourEvent(UserContext me, Task task) {
		super(me, task);
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("CWSCBFZR2")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR2",SystemConfig._cxddfqlcConf.getJd5())&&!isSkip(hashMap, "CWSCBFZR2",SystemConfig._cxddfqlcConf.getJd5()))){
	...
	public  List<UserContext> getNextUserList(){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("CWSCBFZR2")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR2",nextStepId)&&!isSkip(hashMap, "CWSCBFZR2",nextStepId))){
