...
public class zqbLuyouJumpCXDDThreeEvent  extends SysJumpTriggerEvent{

	public zqbLuyouJumpCXDDThreeEvent(UserContext me, Task task) {
		super(me, task);
	}
	/**
	 * 获得下一个办理节点
		 ...
		 if(hash.get("WLINS")!=null&&!hash.get("WLINS").toString().equals("")){
			 Long instanceid=Long.parseLong(hash.get("WLINS").toString());
			   HashMap map=DemAPI.getInstance().getFromData(instanceid, EngineConstants.SYS_INSTANCE_TYPE_DEM);
			    if(map!=null&&map.get("CWSCBFZR2")!=null&&!map.get("CWSCBFZR2").equals("")){
