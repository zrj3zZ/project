 ...
 *
 */
public class zqbLuyouJumpRcywcbEventOne extends SysJumpTriggerEvent {
	public zqbLuyouJumpRcywcbEventOne(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("KHFZR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "KHFZR",SystemConfig._rcywcbLcConf.getJd2())&&!isSkip(hashMap, "KHFZR",SystemConfig._rcywcbLcConf.getJd2()))){
