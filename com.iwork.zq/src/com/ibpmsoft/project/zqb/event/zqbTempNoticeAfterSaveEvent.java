 ...
 *
 */
public class zqbTempNoticeAfterSaveEvent extends ProcessStepTriggerEvent {
	...
	public zqbTempNoticeAfterSaveEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if("".equals(sq)||sq==null){
				...
				Long instanceid = DemAPI.getInstance().newInstance(TEMP_NOTICE_UUID, userid);
				...
				if(!lcbsexists.isEmpty()){
					HashMap hash = lcbsexists.get(0);
			...
			//xlj 2017年3月9日15:08:42 判断董秘起草，下一节点是否是提交发布版6文件，是则短信发送改为完成审核。
			...
			HashMap Map=new HashMap();
			String jSRname = ProcessAPI.getInstance().newTaskId(instanceId).getAssignee();
			Map.put("KHBH", dataMap.get("KHBH").toString());
		    List<HashMap> list=DemAPI.getInstance().getList(this.getConfigUUID("cxddfpbduuid"), Map, null);
		    if(list.size()==1){
				HashMap hashMap = list.get(0);
				String fhspr  = hashMap.get("FHSPR").toString().substring(0,hashMap.get("FHSPR").toString().indexOf("["));
				String zqserver = getConfigUUID("zqServer")==null?"":getConfigUUID("zqServer");
				//判断下一节点是否是专职持续督导
				if(zqserver.equals("sxzq")&&!fhspr.equals(jSRname)){
					//获取抄送人员    
					if(hashMap.get("FHSPR")!=null&&!hashMap.get("FHSPR").toString().equals("")){
						String title = dataMap.get("KHMC").toString() + "提交了公告："+dataMap.get("NOTICENAME").toString()+"，特此通知！";
						ProcessAPI.getInstance().sendProcessCC(this.getActDefId(), this.getActStepId(), this.getTaskId().toString(), instanceId, this.getExcutionId(), title, hashMap.get("FHSPR").toString());
					}
				}
		    }
		}
