...
public class zqbLuyouJumpGGSevenEvent extends SysJumpTriggerEvent {
	public zqbLuyouJumpGGSevenEvent(UserContext me, Task task) {
		super(me, task); 
	}
	/**
	 * 获得下一个办理节点
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(EventUtil.getIsTjfb("isTJFB").equals("1")){
