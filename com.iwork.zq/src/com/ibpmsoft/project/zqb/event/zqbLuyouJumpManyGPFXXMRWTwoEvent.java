...
public class zqbLuyouJumpManyGPFXXMRWTwoEvent extends SysJumpTriggerEvent {

	public zqbLuyouJumpManyGPFXXMRWTwoEvent(UserContext me, Task task) {
		super(me, task);
	} 
	...
	public String getNextStepId() {
		...
		if (l.size() > 0) {
			...
			if (hash.get("OWNER") != null && !hash.get("OWNER").toString().equals("")) {
				if (isExists("FSFZR",hash.get("OWNER").toString())&&!isNextNode("FSFZR",hash.get("OWNER").toString().substring(0,hash.get("OWNER").toString().indexOf("[")))&&!isSkip("FSFZR",hash.get("OWNER").toString().substring(0,hash.get("OWNER").toString().indexOf("[")))) {
	...
	public List<UserContext> getNextUserList() {
		...
		if (l.size() > 0) {
			...
			if (hash.get("OWNER") != null&&!hash.get("OWNER").toString().equals("")) {
				...
				if (list.size() == 1) {
					if (list.get(0).get("FSFZR") != null&& !list.get(0).get("FSFZR").toString().equals("")&&!isNextNode("FSFZR",hash.get("OWNER").toString().substring(0,hash.get("OWNER").toString().indexOf("[")))&&!isSkip("FSFZR",hash.get("OWNER").toString().substring(0,hash.get("OWNER").toString().indexOf("[")))) {
