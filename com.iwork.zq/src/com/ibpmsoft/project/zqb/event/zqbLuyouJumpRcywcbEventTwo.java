 ...
 *
 */
public class zqbLuyouJumpRcywcbEventTwo extends SysJumpTriggerEvent {
	public zqbLuyouJumpRcywcbEventTwo(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(!flag && !flags && "hlzq".equals(gszq)){
