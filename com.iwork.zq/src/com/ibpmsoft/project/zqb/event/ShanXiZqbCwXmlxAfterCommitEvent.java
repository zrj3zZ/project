 ...
 * 
 */
public class ShanXiZqbCwXmlxAfterCommitEvent extends ProcessStepTriggerEvent {
	
	private final static String CN_FILENAME = "/common.properties";
	private static Logger logger = Logger.getLogger(ShanXiZqbCwXmlxAfterCommitEvent.class);
	public ShanXiZqbCwXmlxAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(!dataMap2.isEmpty()){
				...
				content.append(customername).append("已提交一般性财务顾问项目立项，请审核!");
				...
				if(SystemConfig._cwXmlxSpLcConf.getJd3().equals(actStepId)){
					deleteData(processActDefId,SystemConfig._cwXmlxSpLcConf.getJd3(),userid,instanceId);
				}
				...
				if (target != null) {
					if (!content.equals("")) {
						...
						if (email != null && !email.equals("") && ut != null) {
							...
							MessageAPI.getInstance().sendSysMail(senduser, email, "一般性财务顾问项目立项审核", content.toString());
						}
					}
				}
			}
