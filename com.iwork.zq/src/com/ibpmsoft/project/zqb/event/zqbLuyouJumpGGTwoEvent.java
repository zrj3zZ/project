 ...
 *
 */
public class zqbLuyouJumpGGTwoEvent extends SysJumpTriggerEvent {
	public zqbLuyouJumpGGTwoEvent(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("ZZCXDD")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "ZZCXDD",SystemConfig._ggsplcConf.getJd3())&&!isSkip(hashMap, "ZZCXDD",SystemConfig._ggsplcConf.getJd3()))){
