 ...
 *
 */
public class zqbRcywcbAfterSaveEvent extends ProcessStepTriggerEvent {

	private static final String CN_FILENAME = "/common.properties"; //抓取网站配置文件     
	public zqbRcywcbAfterSaveEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if("".equals(sq)||sq==null){
				...
				hashdata.put("LCBH", dataMap.get("LCBH")==null?"":dataMap.get("LCBH").toString());
			...
			if(zqserver.equals("sxzq")){
				//获取抄送人员    
				conditionMap.put("KHBH", dataMap.get("KHBH").toString());
				List<HashMap> list=DemAPI.getInstance().getList(this.getConfigUUID("cxddfpbduuid"), conditionMap, null);
				for(HashMap CSRY:list){
					if(CSRY.get("FHSPR")!=null&&!CSRY.get("FHSPR").toString().equals("")){
						String title = dataMap.get("KHMC").toString() + "提交了："+dataMap.get("SXMC").toString()+"的事项，特此通知！";
						ProcessAPI.getInstance().sendProcessCC(this.getActDefId(), this.getActStepId(), this.getTaskId().toString(), instanceId, this.getExcutionId(), title, CSRY.get("FHSPR").toString());
					}
				}
			}
		}
