 ...
 *
 */
public class zqbLuyouJumpGGFourEvent extends SysJumpTriggerEvent {
	public zqbLuyouJumpGGFourEvent(UserContext me, Task task) {
		super(me, task); 
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("ZZSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "ZZSPR",SystemConfig._ggsplcConf.getJd5())&&!isSkip(hashMap, "ZZSPR",SystemConfig._ggsplcConf.getJd5()))){
