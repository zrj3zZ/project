 ...
 * 
 */
public class ShanXiZqbLuyouJumpBgFangAnBSEvent extends SysJumpTriggerEvent {
	private static Logger logger = Logger.getLogger(ShanXiZqbLuyouJumpBgFangAnBSEvent.class);
	public ShanXiZqbLuyouJumpBgFangAnBSEvent(UserContext me, Task task) {
		super(me, task);
	}
	...
	public String getNextStepId(){
		...
		if(actStepId.equals(SystemConfig._bgFangAnBSSpLcConf.getJd1())){
			if(!spry.get("NODE1").toString().equals("")){
				return SystemConfig._bgFangAnBSSpLcConf.getJd2();
			}else if(!spry.get("NODE2").toString().equals("")){
				return SystemConfig._bgFangAnBSSpLcConf.getJd3();
			}else if(!spry.get("NODE3").toString().equals("")){
				return SystemConfig._bgFangAnBSSpLcConf.getJd5();
			}else{
				return SystemConfig._bgFangAnBSSpLcConf.getEnd();
			}
		}else if(actStepId.equals(SystemConfig._bgFangAnBSSpLcConf.getJd2())){
		...
		}else if(actStepId.equals(SystemConfig._bgFangAnBSSpLcConf.getJd3())){
			if(!spry.get("NODE2").toString().equals("")&&!EventUtil.checkUser(spry.get("NODE1").toString())){
				return SystemConfig._bgFangAnBSSpLcConf.getJd4();
			}else if(!spry.get("NODE3").toString().equals("")){
				return SystemConfig._bgFangAnBSSpLcConf.getJd5();
			}else{
				return SystemConfig._bgFangAnBSSpLcConf.getEnd();
			}
		}else if(actStepId.equals(SystemConfig._bgFangAnBSSpLcConf.getJd4())){
	...
	public HashMap<String, String> getSpry(Long instanceId) {
		StringBuffer sql=new StringBuffer("SELECT (SELECT BGCZ.OWNER FROM BD_ZQB_BGFAZLBS FAZLBS INNER JOIN BD_ZQB_BGZZLXXX BGCZ ON FAZLBS.XMBH=BGCZ.XMBH WHERE FAZLBS.LCBS = ?) AS NODE1,NODE1 AS NODE2,NODE2 AS NODE3,EXTENDSFSP,NODE1SFSP AS NODE2SFSP,NODE2SFSP AS NODE3SFSP FROM BD_ZQB_SXSPRYSZ WHERE LCMC='方案资料报审' AND XMLX='并购项目'");
