...
public class ShanXiBgSbzlEndEvent extends ProcessTriggerEvent {
	
	private final static String CN_FILENAME = "/common.properties";
	private static Logger logger = Logger.getLogger(ShanXiBgSbzlEndEvent.class);
	public ShanXiBgSbzlEndEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(!dataMap2.isEmpty()){
				...
				content.append(jyf).append("并购申报资料审核通过！");
	...
	public HashMap<String,Object> getDataMap(String xmbh,Long dqjdbh){
		...
		sql.append("SELECT BGCZ.ID,BGCZ.JYF,BGCZ.INSTANCEID,NVL(BGCZ.JDBH,0) JDBH,NVL(XMINFO.SORTID,0) SORTID,(SELECT SORTID FROM BD_ZQB_TYXM_INFO WHERE ID = ?) LCSORTID FROM BD_ZQB_BGSBZL FAZLBS INNER JOIN BD_ZQB_BGZZLXXX BGCZ ON FAZLBS.XMBH=BGCZ.XMBH LEFT JOIN BD_ZQB_TYXM_INFO XMINFO ON BGCZ.JDBH=XMINFO.ID WHERE FAZLBS.XMBH = ?");
