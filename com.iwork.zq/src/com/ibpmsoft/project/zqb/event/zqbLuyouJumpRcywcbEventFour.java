 ...
 *
 */
public class zqbLuyouJumpRcywcbEventFour extends SysJumpTriggerEvent {
	public zqbLuyouJumpRcywcbEventFour(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("ZZSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "ZZSPR",SystemConfig._rcywcbLcConf.getJd5())&&!isSkip(hashMap, "ZZSPR",SystemConfig._rcywcbLcConf.getJd5()))){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("ZZSPR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "ZZSPR",nextStepId)&&!isSkip(hashMap, "ZZSPR",nextStepId))){
					 ...
					 uclist = EventUtil.getUcList(uclist, collocationUSERID,nextStepId,excutionId,actdefid);
				 }else if(isExists("FBBWJSHR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FBBWJSHR",nextStepId))){
