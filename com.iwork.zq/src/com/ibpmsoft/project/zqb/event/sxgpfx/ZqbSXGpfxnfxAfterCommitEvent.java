 ...
 * 
 */
public class ZqbSXGpfxnfxAfterCommitEvent extends ProcessStepTriggerEvent {
	
	public ZqbSXGpfxnfxAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			content.append("股票发行项目:").append(projectname).append(",拟发行信息已提交,请审核!");
			...
			if(SystemConfig._xmlxnfxLcConf.getJd3().equals(actStepId)){
				deleteData(processActDefId,SystemConfig._xmlxnfxLcConf.getJd3(),userid,instanceId);
			}
			if (target != null) {
				if (!content.equals("")) {
