 ...
 * 
 */
public class ZqbSXGpfxfazlbsAfterCommitEvent extends ProcessStepTriggerEvent {
	
	public ZqbSXGpfxfazlbsAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			content.append("股票发行项目:").append(projectname).append(",方案资料报审信息已提交,请审核!");
			...
			if(SystemConfig._fazlbsLcConf.getJd3().equals(actStepId)){
				deleteData(processActDefId,SystemConfig._fazlbsLcConf.getJd3(),userid,instanceId);
			}
			if (target != null) {
				if (!content.equals("")) {
					...
					if (email != null && !email.equals("") && ut != null) {
						String senduser = ut.get_userModel().getUsername();
						 MessageAPI.getInstance().sendSysMail(senduser, email, "方案资料报审信息审批", content.toString());
					}
				}
			}
	...
	private void deleteData(String processActDefId, String actStepId, String userid,Long instanceId) {
		...
		int num = com.iwork.core.db.DBUtil.executeUpdate(sql.toString());
	}
}
