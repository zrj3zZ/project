 ...
 * 
 */
public class ZqbSXGpfxsjfxAfterCommitEvent extends ProcessStepTriggerEvent {
	
	public ZqbSXGpfxsjfxAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			content.append("股票发行项目:").append(projectname).append(",实际发行信息已提交,请审核!");
			String assignee = newTaskId.getAssignee();
			//设置BPM专员n
			String actstepid = this.getActStepId();
			if(SystemConfig._zlgdsjfxLcConf.getJd1().equals(actstepid)){
				...
				if (!content.equals("")) {
					...
					if (email != null && !email.equals("") && ut != null) {
						String senduser = ut.get_userModel().getUsername();
						 MessageAPI.getInstance().sendSysMail(senduser, email, "实际发行信息审批", content.toString());
					}
				}
			}
