...
public class zqbLuyouJumpGGEightEvent extends SysJumpTriggerEvent {
	public zqbLuyouJumpGGEightEvent(UserContext me, Task task) {
		super(me, task); 
	}
	/**
	 * 获得下一个办理节点
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("FBBWJSHR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FBBWJSHR"))){
	...
	public  List<UserContext> getNextUserList(){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 boolean isActiveNextNodeFunction = EventUtil.isActiveNextNodeFunction();
				 if(isExists("FBBWJSHR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "FBBWJSHR"))){
					 UserContext uc = UserContextUtil.getInstance().getUserContext(list.get(0).get("FBBWJSHR").toString().substring(0,list.get(0).get("FBBWJSHR").toString().indexOf("[")));
					 uclist.add(uc);
				 }else if(isExists("GGFBR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "GGFBR"))){
