 ...
 * 
 */
public class zqbLuyouJumpXMRWThreeEvent extends SysJumpTriggerEvent {

	public zqbLuyouJumpXMRWThreeEvent(UserContext me, Task task) {
		super(me, task);
	}
	...
	public List<UserContext> getNextUserList() {
		...
		List<HashMap> list = DemAPI.getInstance().getList(demUUID,
				conditionMap, null);
		if (list.size() == 1) {
			if (list.get(0).get("ZZSPR") != null
					&& !list.get(0).get("ZZSPR").toString().equals(
							"")&&!isNextNode("ZZSPR")&&!isSkip("ZZSPR")) {
