 ...
 *
 */
public class zqbLuyouJumpGGEvent extends SysJumpTriggerEvent {
	public zqbLuyouJumpGGEvent(UserContext me, Task task) {
		super(me, task);
	}
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("QYNBRYSH")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "QYNBRYSH",SystemConfig._ggsplcConf.getJd0())&&!isSkip(hashMap, "QYNBRYSH",SystemConfig._ggsplcConf.getJd0()))){
					 return SystemConfig._ggsplcConf.getJd0();
				 }else if(isExists("KHFZR")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "KHFZR",SystemConfig._ggsplcConf.getJd2())&&!isSkip(hashMap, "KHFZR",SystemConfig._ggsplcConf.getJd2()))){
