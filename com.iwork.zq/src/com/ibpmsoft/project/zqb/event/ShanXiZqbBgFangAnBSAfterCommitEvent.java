 ...
 * 
 */
public class ShanXiZqbBgFangAnBSAfterCommitEvent extends ProcessStepTriggerEvent {
	
	private final static String CN_FILENAME = "/common.properties";
	private static Logger logger = Logger.getLogger(ShanXiZqbBgFangAnBSAfterCommitEvent.class);
	public ShanXiZqbBgFangAnBSAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(!dataMap2.isEmpty()){
				...
				content.append(jyf).append("已提交并购方案资料报审，请审核!");
				...
				if(SystemConfig._bgFangAnBSSpLcConf.getJd3().equals(actStepId)){
					deleteData(processActDefId,SystemConfig._bgFangAnBSSpLcConf.getJd3(),userid,instanceId);
				}
				...
				if (target != null) {
					if (!content.equals("")) {
						...
						if (email != null && !email.equals("") && ut != null) {
							...
							MessageAPI.getInstance().sendSysMail(senduser, email, "并购方案资料报审审核", content.toString());
						}
					}
				}
			}
