 ...
 * 
 */
public class ShanXiZqbGuGaiAfterCommitEvent extends ProcessStepTriggerEvent {
	
	public ShanXiZqbGuGaiAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			String customername = dataMap.get("YGSMC").toString();
			UserContext uc = UserContextUtil.getInstance().getCurrentUserContext();
			Task newTaskId = ProcessAPI.getInstance().newTaskId(instanceId);
			StringBuffer content=new StringBuffer();
			content.append(customername).append("已提交项目股改，请审核!");
			...
			//删除一条意见记录
			if(SystemConfig._gugaiLcConf.getJd3().equals(actStepId)){
				deleteData(processActDefId,SystemConfig._gugaiLcConf.getJd3(),userid,instanceId);
			}
			if (target != null) {
				if (!content.equals("")) {
					...
					if (email != null && !email.equals("") && ut != null) {
						String senduser = ut.get_userModel().getUsername();
						 MessageAPI.getInstance().sendSysMail(senduser, email, "项目股改审核", content.toString());
					}
				}
			}
