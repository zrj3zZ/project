...
public class zqbLuyouJumpManyXMRWThreeEvent extends SysJumpTriggerEvent {

	public zqbLuyouJumpManyXMRWThreeEvent(UserContext me, Task task) {
		super(me, task);
	}
	...
	public String getNextStepId() {
		...
		if (l.size() > 0) {
			...
			if (hash.get("OWNER") != null&& !hash.get("OWNER").toString().equals("")){
				if (isExists("ZZSPR", hash.get("OWNER").toString())&&!isNextNode("ZZSPR",hash.get("OWNER").toString().substring(0,hash.get("OWNER").toString().indexOf("[")))&&!isSkip("ZZSPR")) {
	...
	public List<UserContext> getNextUserList() {
		...
		if (l.size() > 0) {
			...
			if (hash.get("OWNER") != null&& !hash.get("OWNER").toString().equals("")) {
				...
				if (list.size() == 1) {
					if (list.get(0).get("ZZSPR") != null&& !list.get(0).get("ZZSPR").toString().equals("")&&!isNextNode("ZZSPR",hash.get("OWNER").toString().substring(0,hash.get("OWNER").toString().indexOf("[")))&&!isSkip("ZZSPR")) {
