 ...
 * 
 */
public class ShanXiZqbCwGzjdhbAfterCommitEvent extends ProcessStepTriggerEvent {
	private static Logger logger = Logger.getLogger(ShanXiZqbCwGzjdhbAfterCommitEvent.class);
	private final static String CN_FILENAME = "/common.properties";
	
	public ShanXiZqbCwGzjdhbAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(!dataMap2.isEmpty()){
				...
				content.append(customername).append("已提交一般财务工作进度汇报，请审核!");
				...
				if(SystemConfig._cwGzjdhbSpLcConf.getJd3().equals(actStepId)){
					deleteData(processActDefId,SystemConfig._cwGzjdhbSpLcConf.getJd3(),userid,instanceId);
				}
