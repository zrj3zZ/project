 ...
 * 
 */
public class ShanXiZqbLuyouJumpBgSbzlEvent extends SysJumpTriggerEvent {
	private static Logger logger = Logger.getLogger(ShanXiZqbLuyouJumpBgSbzlEvent.class);
	public ShanXiZqbLuyouJumpBgSbzlEvent(UserContext me, Task task) {
		super(me, task);
	}
	...
	public String getNextStepId(){
		...
		if(actStepId.equals(SystemConfig._bgSbzlSpLcConf.getJd1())){
			if(!spry.get("NODE1").toString().equals("")){
				return SystemConfig._bgSbzlSpLcConf.getJd2();
			}else if(!spry.get("NODE2").toString().equals("")){
				return SystemConfig._bgSbzlSpLcConf.getJd3();
			}else if(!spry.get("NODE3").toString().equals("")){
				return SystemConfig._bgSbzlSpLcConf.getJd5();
			}else if(!spry.get("NODE4").toString().equals("")){
				return SystemConfig._bgSbzlSpLcConf.getJd6();
			}else{
				return SystemConfig._bgSbzlSpLcConf.getEnd();
			}
		}else if(actStepId.equals(SystemConfig._bgSbzlSpLcConf.getJd2())){
		...
		}else if(actStepId.equals(SystemConfig._bgSbzlSpLcConf.getJd3())){
			if(!spry.get("NODE2").toString().equals("")&&!EventUtil.checkUser(spry.get("NODE1").toString())){
				return SystemConfig._bgSbzlSpLcConf.getJd4();
			}else if(!spry.get("NODE3").toString().equals("")){
				return SystemConfig._bgSbzlSpLcConf.getJd5();
			}else if(!spry.get("NODE4").toString().equals("")){
				return SystemConfig._bgSbzlSpLcConf.getJd6();
		...
		}else if(actStepId.equals(SystemConfig._bgSbzlSpLcConf.getJd4())){
		...
		}else if(actStepId.equals(SystemConfig._bgSbzlSpLcConf.getJd5())){
	...
	public HashMap<String, String> getSpry(Long instanceId) {
		StringBuffer sql=new StringBuffer("SELECT "
				+ "(SELECT BGCZ.OWNER FROM BD_ZQB_BGSBZL FAZLBS INNER JOIN BD_ZQB_BGZZLXXX BGCZ ON FAZLBS.XMBH=BGCZ.XMBH WHERE FAZLBS.LCBS = ?) AS NODE1,"
				+ "NODE1 AS NODE2,"
				+ "NODE2 AS NODE3,"
				+ "NODE3 AS NODE4,"
				...
				+ "NODE3SFSP AS NODE4SFSP FROM BD_ZQB_SXSPRYSZ WHERE LCMC='申报资料' AND XMLX='并购项目'");
