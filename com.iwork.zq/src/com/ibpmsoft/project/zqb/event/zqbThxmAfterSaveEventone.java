 ...
 *
 */
public class zqbThxmAfterSaveEventone extends ProcessStepTriggerEvent {

	private static final String CN_FILENAME = "/common.properties"; //抓取网站配置文件     
	public zqbThxmAfterSaveEventone(UserContext uc, HashMap hash) {
		super(uc, hash);
				...
				if(lcbsexists!=null&&lcbsexists.size()==1){
					...
					for(HashMap<String,Object> hash:lcbsexists){
						...
						if(hash.get("ZBSCBJBR").equals("")){
							...
							hash.put("ZBSCBJBR", assignee+"["+target._userModel.getUsername()+"]");
							dataMap.put("ZBSCBJBR", assignee);
						}else{
							hash.put("ZBSCBJBR", hash.get("ZBSCBJBR"));
						}
