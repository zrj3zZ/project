...
public class zqbLuyouJumpGGDUFiveEvent  extends SysJumpTriggerEvent{
	public zqbLuyouJumpGGDUFiveEvent(UserContext me, Task task) {
		super(me, task);
	}
	/**
	 * 获得下一个办理节点
		...
		HashMap<String,Object>  hash = ProcessAPI.getInstance().getFromData(this.getInstanceId(), EngineConstants.SYS_INSTANCE_TYPE_PROCESS);
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("CWSCBFZR3")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR3",SystemConfig._cxddfqlcConf.getJd6())&&!isSkip(hashMap, "CWSCBFZR3",SystemConfig._cxddfqlcConf.getJd6()))){
	...
	public  List<UserContext> getNextUserList(){
		 ...
		 if(hash.get("KHBH")!=null&&!hash.get("KHBH").toString().equals("")){
			 ...
			 if(list.size()==1){
				 ...
				 if(isExists("CWSCBFZR3")&&(!isActiveNextNodeFunction?true:!isNextNode(hashMap, "CWSCBFZR3",nextStepId)&&!isSkip(hashMap, "CWSCBFZR3",nextStepId))){
