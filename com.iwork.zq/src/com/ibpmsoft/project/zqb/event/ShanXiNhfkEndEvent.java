...
public class ShanXiNhfkEndEvent extends ProcessTriggerEvent {
	public ShanXiNhfkEndEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			content.append(dataMap.get("CUSTOMERNAME").toString()).append("内核反馈审核通过！");
			...
			if (target != null && tg != null) {
				...
				if (email != null && !email.equals("")) {
					MessageAPI.getInstance().sendSysMail(senduser,email, "内核反馈审核", content.toString());
				}
			}
		}
		return flag;
