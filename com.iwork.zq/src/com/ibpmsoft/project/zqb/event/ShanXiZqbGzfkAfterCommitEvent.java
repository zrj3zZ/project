 ...
 * 
 */
public class ShanXiZqbGzfkAfterCommitEvent extends ProcessStepTriggerEvent {
	
	public ShanXiZqbGzfkAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			content.append(customername).append("已提交股转反馈及回复，请审核!");
			...
			if(SystemConfig._gzfkLcConf.getJd3().equals(actStepId)){
				deleteData(processActDefId,SystemConfig._gzfkLcConf.getJd3(),userid,instanceId);
			}
			if (target != null) {
				if (!content.equals("")) {
					...
					if (email != null && !email.equals("") && ut != null) {
						String senduser = ut.get_userModel().getUsername();
						 MessageAPI.getInstance().sendSysMail(senduser, email, "股转反馈及回复审核", content.toString());
					}
				}
			}
