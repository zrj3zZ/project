 ...
 *
 */
public class zqbDzwthProAfterRejectEvent extends ProcessStepTriggerEvent {

	public zqbDzwthProAfterRejectEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(flag){
				...
				String content = ZQBNoticeUtil.getInstance().getNoticeSmsContent(ZQB_Notice_Constants.DZWTH_REJECT_KEY, contentMap);
				if (target != null) {
					if (!content.equals("")) {
						//发送短信
						...
						if (email != null && !email.equals("")) {
							...
							MessageAPI.getInstance().sendSysMail(senduser, email, "定增(200人以内)流程审核", content.toString());
						}
					}
				}
