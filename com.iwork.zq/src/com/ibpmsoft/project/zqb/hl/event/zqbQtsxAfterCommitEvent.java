...
public class zqbQtsxAfterCommitEvent extends ProcessStepTriggerEvent{

	public zqbQtsxAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(flag){
				...
				//${USERNAME}提交了${SXMC}事项的审批，请在其他审批流程中查看。
				contentMap.put("USERNAME", username);
				contentMap.put("SXMC", dataMap.get("SXMC").toString());
				String content = ZQBNoticeUtil.getInstance().getNoticeSmsContent(ZQB_Notice_Constants.QTLC_COMMIT_KEY, contentMap);
