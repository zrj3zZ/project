 ...
 * 
 */
public class zqbDzwthProLuyouJumpYjghEvent extends SysJumpTriggerEvent {
	public zqbDzwthProLuyouJumpYjghEvent(UserContext me, Task task) {
		super(me, task);
	}
	...
	public String getNextStepId() {
		...
		List<HashMap> userList = DBUtil.getDataList(lables, "SELECT SUBSTR(B.MANAGER,0,INSTR(B.MANAGER,'[',1)-1) AS MANAGER,SUBSTR(B.OWNER,0,INSTR(B.OWNER,'[',1)-1) AS OWNER FROM BD_ZQB_GPFXXMB B WHERE B.PROJECTNO=?", params);
		String manager = "";
		String owner = "";
		if(userList.size()==1){
		...
		if(actStepId.equals(SystemConfig._hlDzyjghLcConf.getJd1())&&manager != null && !manager.toString().equals("")){
			return SystemConfig._hlDzyjghLcConf.getJd2();
		}else if(actStepId.equals(SystemConfig._hlDzyjghLcConf.getJd2())&&owner != null && !owner.toString().equals("")){
			return SystemConfig._hlDzyjghLcConf.getJd3();
		}else{
			return SystemConfig._hlDzyjghLcConf.getJd4();
		}
	}
