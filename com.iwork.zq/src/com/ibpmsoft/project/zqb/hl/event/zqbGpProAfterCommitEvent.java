 ...
 *
 */
public class zqbGpProAfterCommitEvent extends ProcessStepTriggerEvent {

	public zqbGpProAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	}
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(flag){
				...
				String content = ZQBNoticeUtil.getInstance().getNoticeSmsContent(ZQB_Notice_Constants.GP_COMMIT_KEY, contentMap);
				...
				if (target != null) {
					if (!content.equals("")) {
						...
						if (email != null && !email.equals("")) {
							...
							MessageAPI.getInstance().sendSysMail(senduser, email, "挂牌项目流程审核", content.toString());
						}
					}
				}
