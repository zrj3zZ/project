 ...
 *
 */
public class zqbDzwothProAfterCommitEvent extends ProcessStepTriggerEvent {

	public zqbDzwothProAfterCommitEvent(UserContext uc, HashMap hash) {
		super(uc, hash);
	...
	public boolean execute() {
		...
		if(dataMap!=null){
			...
			if(flag){
				...
				String content = ZQBNoticeUtil.getInstance().getNoticeSmsContent(ZQB_Notice_Constants.DZWOTH_COMMIT_KEY, contentMap);
				...
				if (target != null) {
					if (!content.equals("")) {
						...
						if (email != null && !email.equals("")) {
							...
							MessageAPI.getInstance().sendSysMail(senduser, email, "定增(200人以上)流程审核", content.toString());
						}
					}
				}
