 ...
 * 
 */
public class ZQB_ItemBz_BaseAddEvent extends DemTriggerEvent {
	public ZQB_ItemBz_BaseAddEvent(UserContext me, HashMap hash) {
		super(me, hash);
	}
	...
	public boolean execute() {
		...
		String bznr=hash.get("BZNR").toString();
		Long dataId = Long.parseLong(hash.get("ID").toString());
		if(!"0".equals(map.get("instanceId").toString())){
			LogUtil.getInstance().addLog(dataId, "信披事项步骤信息维护", "更新信披事项ID为"+sxid+"的信披步骤内容:"+bznr);
		}else{
			LogUtil.getInstance().addLog(dataId, "信披事项步骤信息维护", "新增信披事项ID为"+sxid+"的信披步骤内容:"+bznr);
		}
		return true;
	}
}
