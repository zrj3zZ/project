...
public class ZQB_NHDATE_SaveEvent extends DemTriggerEvent{
	private static Logger logger = Logger.getLogger(ZQB_NHDATE_SaveEvent.class);
	public final static String CN_FILENAME = "/common.properties";// 抓取网站配置文件
	public ZQB_NHDATE_SaveEvent(UserContext me,HashMap hash){
		super(me,hash);
	}
	...
	public boolean execute(){
		...
		String nhdate=map.get("NHDATE")==null?"":map.get("NHDATE").toString();
		boolean flag=false;
		if(!nhdate.equals("")&&jlsj.equals("")){
			flag=true;
		}else if(!jlsj.equals("")&&!nhdate.equals("")&&!jlsj.equals(nhdate)){
			flag=true;
		}
		if(flag){
			...
			map2.put("NHDATE", hash.get("NHDATE").toString());
			smsContent = ZQBNoticeUtil.getInstance().getNoticeSmsContent(ZQB_Notice_Constants.NHFK_ADD_KEY, map2);
			String user=list.get(0).get("MANAGER").toString().substring(0,list.get(0).get("MANAGER").toString().indexOf("["));
			...
			Date d = null;
			try {
				d = df.parse(clone.get("NHDATE").toString());
			...
			map3.put("NHDATE", clone.get("NHDATE").toString());
			...
			class DXThread extends Thread {
				...
				public void run() { 
					...
					public void run() {
						...
						String uuid=clone.get("NHUUID").toString();
						List<HashMap> list = DemAPI.getInstance().getList(clone.get("NHUUID").toString(), hashmap, null);
