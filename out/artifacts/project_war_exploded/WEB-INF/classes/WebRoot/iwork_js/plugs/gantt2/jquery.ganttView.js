...
(function (jQuery) {
	...
	var Chart = function(div, opts) {
        ...
        function addBlocks(div, data, cellWidth, start) {
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
					...
					var memo = series.memo;
					var manager = series.manager;
					var block = jQuery("<div>", {
                        "class": "ganttview-block",
                        "title":"任务名称："+ series.name + "\n任务天数：" + size + " 天"+ "\n完成率：" + scale + " %"+ "\n负责人：" + manager + " "+ "\n备注：" + memo + " ",
                        "css": {
                    ...
                    if(size>2){
                    	block.append(jQuery("<div>", { "class": "ganttview-block-text" }).html(text));
                    }else{
                    	block.append(jQuery("<div>", { "class": "ganttview-block-text" }).html("..."));
                    }
        ...
        function addBlockData(block, data, series) {
        	...
        	var blockData = { id: data.id, name: data.name ,text:data.text};
	...
	var Behavior = function (div, opts) {
		function apply() {
			if(opts.readonly){
	            ...
	            if (opts.behavior.dbclickable) { 
	            	bindBlockDbClick(div, opts.behavior.onDblClick); 
	            }
	            if (opts.behavior.contextmenuable) { 
	            	bindBlockContextMenu(div, opts.behavior.onContextMenu); 
	            }
			}
		}
        ...
        function bindBlockDbClick(div, callback) {
        	jQuery("div.ganttview-block", div).live("dblclick", function () {
        		if (callback) { callback(jQuery(this).data("block-data")); } 
        	});
        }
        function bindBlockContextMenu(div, callback) {
        	jQuery("div.ganttview-block", div).live("contextmenu", function () {
        		if (callback) { callback(jQuery(this).data("block-data")); } 
        	});
        }
        ...
        function updateDataAndPosition(div, block, cellWidth, startDate) {
			...
			var blockData = block.data("block-data");
			 var size = DateUtils.daysBetween(blockData.start, blockData.end) + 1;
			 
			 if(size>2){
				 jQuery("div.ganttview-block-text", block).html(blockData.text);
             }else{
            	 jQuery("div.ganttview-block-text", block).html("...");
             }
