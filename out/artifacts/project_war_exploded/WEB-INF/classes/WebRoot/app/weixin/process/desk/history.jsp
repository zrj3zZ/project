...
<script>
  <s:property value="initWeiXinScript" escapeHtml="false"/>
   $(document).on("pageinit","#pageone",function(){
		  $("#searchInput").on( "change", function(event, ui) {
		  		...
		  		var pageUrl = "weixin_processdeskHistory.action?key="+key;
		  		redirectUrl(pageUrl);
		  });
  ...
  function showMore(pageNo,key){
	  ...
	  if(key==''){
	  	...
	  	alert(key);
  	  $.post('weixin_processdeskHistoryHtml.action',{pageNo:pageNo,key:key}, function (text, status) {
  			$("#moreLi").remove();
    ...
    <div data-role="footer"  data-position="fixed">
	   <div data-role="navbar">
        <ul>
            ...
            <li><a href="#" class="ui-btn-active" data-icon="star">已办跟踪</a></li>
            <li><a href="javascript:redirectUrl('weixin_processdeskNotice.action')" data-icon="gear"  >传阅/通知</a></li>
        </ul>
    </div><!-- /navbar -->  
	</div><!-- /footer --> 
</div> 
