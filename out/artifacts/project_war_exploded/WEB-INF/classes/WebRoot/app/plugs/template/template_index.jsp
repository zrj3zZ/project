	...
	<script type="text/javascript">
		...
		function initTree(){
			var setting = {
						async: {
							enable: true, 
							url:"hna_sanhui_templateJSON.action",  
							dataType:"json",
		...
		function onClick(event, treeId, treeNode, clickFlag){
			...
			var companyno = treeNode.companyno;
			var companyname = treeNode.companyname;
			var pageUrl = "hna_sanhui_template_list.action?id="+id+"&companyno="+companyno+"&companyname="+companyname+"&nodeType="+nodeType;
		...
		function addTemplateType(){
			...
			if(nodes.length==1){
				var pageUrl = "createFormInstance.action?formid=141&demId=43&FID="+id+"&COMPANYNO="+companyno+"&COMPANYNAME="+encodeURI(companyname);
		...
		function addTemplate(){
			...
			var companyname = nodes[0].companyname;
			if(nodes.length==1){
				var pageUrl = "createFormInstance.action?formid=142&demId=44&TYPE="+id+"&COMPANYNO="+companyno+"&COMPANYNAME="+encodeURI(companyname);
					 art.dialog.open(pageUrl,{ 
						id:'Category_show',
						cover:true, 
						title:'新增模板',
		...
		function editTemplateType(){
			...
			if(nodes.length==1){
				var pageUrl = "openFormPage.action?formid=141&demId=43&instanceId="+instanceid+"&COMPANYNO="+companyno+"&COMPANYNAME="+companyname;
					 art.dialog.open(pageUrl,{ 
						id:'Category_show',
						cover:true,
						title:'编辑模板分类',
		...
		function deleteTemplateType(){
			...
			if(confirm("确定要删除模板类别吗？")){
				var pageUrl="hna_sanhui_templatetype_delete.action";
...
<body class="easyui-layout">
    ...
    <div region="center" border="false"  align="center" style="border:0px solid #C0C0C0;">
    	<iframe id="mainFrame" name="mainFrame" scrolling="no" frameborder="0"  src="hna_sanhui_template_list.action" style="width:100%;height:100%;"></iframe> 
    </div>
