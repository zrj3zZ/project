	...
	$(function(){
		$("#pp").pagination({
			...
			onSelectPage:function(pageNumber, pageSize){
				var pageUrl="iwork_mail_send_index.action?pageSize="+pageSize+"&pageNumber="+pageNumber;
					window.location.href = pageUrl;
				}
			});
		});
      	function showDetailInfo(id,taskid,title){
      		var emailType = '<s:property value="emailType"/>';
      		if(title.length>5){
	   ...
	   function totalDel(){		
			...
			if(ids!=""){
						if(confirm("确认要删除？")){
							...
							var url ='iwork_mail_send_clean.action?id='+ids;
		 					var option={
							type: "post",
							url: url,
							data: "",
   ...
   function del(){		
			...
			if(counts.length>0){
				...
				if(ids!=""){
					if(confirm("确认要删除？")){
						ids = ids.substring(0,ids.lastIndexOf(','));
						var url ='iwork_mail_send_del.action?id='+ids;
		 				var option={
							type: "post",
							url: url,
							data: "",
...
//邮件标星
   function setStar(){		
			...
			if(counts.length>0){
				...
				if(ids!=""){
					ids = ids.substring(0,ids.lastIndexOf(','));
					var url ='iwork_mail_send_star.action?id='+ids;
		 			var option={
						type: "post",
						url: url,
						data: "",
		...
		//取消标星
		 function cancelSetStar(){		
			...
			if(counts.length>0){
				...
				if(ids!=""){
					ids = ids.substring(0,ids.lastIndexOf(','));
					var url ='iwork_mail_send_restar.action?id='+ids;
		 			var option={
						type: "post",
						url: url,
						data: "",
            ...
            <div region="center" border="false" style="background: #fff; border:1px solid #fff">
            	<table width="100%" cellspacing="0" cellpadding="0" border="0">
            		<tr>
		 				...
		 				<td class="title" style="width:120px;" >收件人</td>
		 			  ...
		 			  <s:iterator value="ownerlist" status="status">
		 				<s:if test="isRead==1">
				 			<tr class="mailItem" id="item_<s:property value="id"/>"  name="receiveMail">
				 				<td ><input type="checkbox" name="ckb_selectAll"; value="<s:property value="id"/>"/></td>
				 				<td  class="mailicon" ><input type="button" class="unread_icon" disabled/></td>
				 				<td  class="mailUser"  onClick="javascript:showDetailInfo('<s:property value="bindId"/>','<s:property value="id"/>','<s:property value="title"/>');" ><s:property value="mailTo" /></td>
				 				<td class="mailTitle"  onClick="javascript:showDetailInfo('<s:property value="bindId"/>','<s:property value="id"/>','<s:property value="title"/>');" ><s:property value="title"/></td>
				 				<td class="mailTitle">
				 				<s:property value="%{getText('{0,date,yyyy-MM-dd HH:mm}',{createTime})}"/></td>
				 				<td class="mailicon"><s:if test="isStar==0 || isStar==null">
							      <img id="bflag" onclick="javascript:clickSetStar('<s:property value="id"/>');" src="iwork_img/star_empty.png"/>
                         </s:if> 
                         <s:if test="isStar==1">
                              <img id="bflag" onclick="javascript:clickCancerStar('<s:property value="id"/>');" src="iwork_img/star_full.png"/>
                        </s:if> </td>
				 			</tr>
		 				</s:if> 
		 				<s:else>
				 			<tr  class="mailItem" id="item_<s:property value="id"/>" name="receiveMail">
				 				<td ><input type="checkbox" name="ckb_selectAll" value="<s:property value="id"/>"/></td>
				 				<td  class="mailicon" ><input type="button" class="unread_icon"  onclick="selectAll()"; /></td>
				 				<td  class="mailUser"  onClick="javascript:showDetailInfo('<s:property value="bindId"/>','<s:property value="id"/>','<s:property value="title"/>');" ><s:property value="mailTo" /></td>
				 				<td class="mailTitle"  onClick="javascript:showDetailInfo('<s:property value="bindId"/>','<s:property value="id"/>','<s:property value="title"/>');" ><s:property value="title"/></span></td>
				 				<td class="mailTitle">
				 				<s:property value="%{getText('{0,date,yyyy-MM-dd HH:mm}',{createTime})}"/></td>
				 			    <td class="mailicon"><s:if test="isStar==0 || isStar==null">
							<img id="bflag" onclick="javascript:clickSetStar('<s:property value="id"/>');" src="iwork_img/star_empty.png"/>
