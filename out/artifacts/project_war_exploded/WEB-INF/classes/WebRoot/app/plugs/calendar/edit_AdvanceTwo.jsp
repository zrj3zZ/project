...
java.util.HashMap map1 = new java.util.LinkedHashMap();
for(int i=1;i<32;i++){
	map1.put(i,i);
}
request.setAttribute("timeMapyue",map1);
%>
<%
...
<title>IWORK综合应用管理系统</title>
		...
		$(document).ready(function(){
			...
			$("#sch_create").bind("click", function(){
				...
				if(document.getElementById("editForm_iworkSchCalendar_reMode1").checked){
					document.getElementById("reWeekDate1").value=$("#reWeekDateVal01").val();
				}
				
				if(document.getElementById("editForm_iworkSchCalendar_reMode2").checked){
					
					document.getElementById("reMonthDays1").value= $("#reMonthDaysVal01").val();

				}
				
				if(document.getElementById("editForm_iworkSchCalendar_reMode3").checked){
					//document.getElementById("nianyue").value=$("#reYearMonthVal01").val();
					
					// document.getElementById("nianri").value=$("#reYearDaysVal01").val();
					
				//	alert(document.getElementById("nianri").value);
					
				
						document.getElementById("reYearMonth1").value=$("#reYearMonthVal01").val();
					
					
				
						document.getElementById("reYearDays1").value=$("#reYearDaysVal01").val();
			...
			$("#choseDate0").bind("click", function(){
				...
				document.getElementById("zqmsdiv2").style.display = "none";
		/* 	alert($("#reStartdate").val());
			alert(document.getElementsByName("iworkSchCalendar.startdate")[1].value); */
			document.getElementsByName("iworkSchCalendar.startdate")[1].value=$("#reStartdate").val();
		//	document.getElementsByName("editForm_iworkSchCalendar_enddate")[0].value=$("#reStartdate").val();
				$("#stauts").val("1");
			});
			//点击开始日期自动选择非周期模式
			...
			$("#sch_save").bind("click", function(){
				...
				if(document.getElementById("editForm_iworkSchCalendar_reMode1").checked){
					if($("#editForm_iworkSchCalendar_reWeekDate").val()=="" || $("#editForm_iworkSchCalendar_reWeekDate").val()==null){
						...
						var obj=document.getElementsByName("reWeekMode");
						for(var i=0; i<obj.length; i++){
							if(obj[i].checked)
								s+=obj[i].value+"_"; //如果选中，将value添加到变量s中
						} 
						if(s==""){
				...
				if(flag){
					 $("#stauts").val("1");
					setRepeateMode(reStartdate,reEnddate,reMode,reDayInterval,s,reMonthDays,reYearMonth,reYearDays);
				}
			});
	...
	function getRepeateModeInfo(reStartdateVal,reEnddateVal,reModeVal,reDayIntervalVal,reWeekDateVal,reMonthDaysVal,reYearMonthVal,reYearDaysVal){
		...
		if(reModeVal==1){
			for(i=0;i<reWeekDateArr.length;i++){
				...
				$("#reWeekDateVal01").val(zq);
			}
		}
		if(reModeVal==2){
			RepeateModeInfo = RepeateModeInfo + "每月第"+reMonthDaysVal+"日,";
			
			$("#reMonthDaysVal01").val(reMonthDaysVal);
		}
		if(reModeVal==3){
			RepeateModeInfo = RepeateModeInfo + "每年"+reYearMonthVal+"月"+reYearDaysVal+"日,";
			$("#reYearMonthVal01").val(reYearMonthVal);
			$("#reYearDaysVal01").val(reYearDaysVal);
		}
	...
	function showxxr(){
	//	window.location.href = "appointmentAdd.action";
	pageUrl="appointmentAdd.action";
		art.dialog.open(pageUrl,{
			title:'创建日程',
			loadingText:'正在加载中,请稍后...',
			rang:true,
...
<body  class="easyui-layout">
	<s:form id="testzzz" name="testzzz" action="saveSchCalendars" theme="simple">
		...
		<s:hidden name="iworkSchCalendar.reWeekDate" id="reWeekDate1" />
		<s:hidden name="iworkSchCalendar.reMonthDays" id="reMonthDays1" />
		<s:hidden name="iworkSchCalendar.reYearMonth" id="reYearMonth1" />
	...
	<div region="center" style="width:530px;height:460px;padding:3px;border:0px;">
            	<s:form id ="editForm" name="editForm" action="saveSchCalendar"  theme="simple">
	            	<table border="0"  cellspacing="0" cellpadding="0" width="100%">
	            		<tr>
	            			<td class="td_title" style="text-align:center;" width="5%">事项描述:</td>
	            			<td class="td_data" width="95%" style="text-align: left;"> 
	            			<input id="title" name="iworkSchCalendar.title" style="width: 95%;" maxlength="32"/>	
	            			<input type="hidden" value="" id="reMonthDaysVal01">
	            			<input type="hidden" value="" id="reYearMonthVal01">
	            			<input type="hidden" value="" id="reYearDaysVal01">
	            			<input type="hidden" value="" id="reWeekDateVal01">
	            				<%-- <s:textarea name="iworkSchCalendar.title" cssStyle="min-width:420px;min-height:100px;" theme="simple"></s:textarea> --%>
	            			</td>
	            		</tr>
	            		<tr>
	           				<td colspan="2">
	            	...
	            	<table border="0"  cellspacing="0" cellpadding="0" width="100%">
		            					...
		            					<tr>
		            						...
		            						<td width="75%">
		            							<div id="reMode0" style="display: block;padding-left: 10px;padding-top:10px">
		            								每<s:textfield  onkeyup="value=(parseInt((value=value.replace(/\D/g,''))==''||parseInt((value=value.replace(/\D/g,''))==0)?'1':value,10))" onafterpaste="value=(parseInt((value=value.replace(/\D/g,''))==''||parseInt((value=value.replace(/\D/g,''))==0)?'1':value,10))" name="iworkSchCalendar.reDayInterval" cssStyle="width:20px;"></s:textfield>转让日
		            								&nbsp;&nbsp;
		            								<input   type="button" value="设置休息日"  onclick="showxxr()" />
		            							</div>
		            							...
		            							<div id="reMode2" style="display: none;padding-left: 10px;padding-top:10px">
		            								每月&nbsp;第<s:select name="iworkSchCalendar.reMonthDays" list="#request.timeMapyue" headerKey="" headerValue=""></s:select>天
		            							</div><%-- <s:textfield onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');" name="iworkSchCalendar.reMonthDays" cssStyle="width:20px;" headerKey="" headerValue=""></s:textfield> --%>
		            							<div id="reMode3" style="display: none;padding-left: 10px;padding-top:10px">
		            								...
		            								&nbsp第<s:select name="iworkSchCalendar.reYearDays" list="#request.timeMapyue" headerKey="" headerValue=""></s:select>天
		            							</div>
		            						</td>
		            					</tr>
		           					</table>
		           				...
		           				<legend id="dateTitle"><font color="808080" >重复范围</font></legend>
		            				<table width="100%">
		            					<tr>
		            						<td width="45%">
			            						<label>开始日期:
				            					<s:textfield id="reStartdate" name="iworkSchCalendar.reStartdate" onclick="WdatePicker({minDate:'#F{$dp.$D(\\'reEnddate\\')}'})" cssStyle="width:100px;" theme="simple">
			            					...
			            					<td width="55%">
		            							...
		            							<label>结束日期:
				            					<s:textfield id="reEnddate" name="iworkSchCalendar.reEnddate" onclick="WdatePicker({minDate:'#F{$dp.$D(\\'reStartdate\\')}'})" cssStyle="width:100px;" theme="simple">
	            		...
	            		<tr>
	            			...
	            			<td class="td_data" width="83%">
	            				...
	            				<input type="hidden" id="stauts" value="1">
	            			</td>
	            		</tr>
	            	</table>
	           				...
	           				<fieldset>
		           				<legend id="alertTitle"><font color="808080" >提醒时间</font></legend>
		            				<table width="100%">
		            					<tr>
			            					<td width="46%">
			            						提前:&nbsp;<s:select id="alerttime1" name="iworkSchCalendar.alerttime" list="#{'0':'0分钟','5':'5分钟','10':'10分钟','15':'15分钟','30':'30分钟','45':'45分钟','60':'1小时','120':'2小时','180':'3小时','360':'6小时','720':'12小时','1080':'18小时','1440':'1天','2880':'2天','10080':'1周'}" cssStyle="width:100px;" headerKey="15" headerValue="15分钟"></s:select>
			            					</td>
			            					<td width="27%">
	            		...
	            		<tr>
	           				...
	           				<fieldset>
		           				<legend id="remarkTitle"><font color="808080" >备注</font></legend>
		            				<table width="100%">
		            					<tr><td><s:textarea id="remark1" name="iworkSchCalendar.remark" cssStyle="width:435px;height:90px;" maxlength="256"/></td></tr>
		           					</table>
		           				</fieldset>
	           				</td>
	            		</tr>
