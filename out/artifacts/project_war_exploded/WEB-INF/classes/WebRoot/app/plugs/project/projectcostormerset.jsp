...
<script type="text/javascript">
	...
	function doSubmit(){
		...
		if(checkvalue!=null&&checkvalue!=''){
			...
			if(name!=''){
				$("#"+name,parent.document).val(chackval[0]);
			}
			if(no!=''){
				$("#"+no,parent.document).val(chackval[1]);
			}
			closeWin();
		}else{
	...
	function doSearch(){
		...
		var seachUrl = encodeURI("zqb_project_costormer_set.action?name="+ customername + "&no=" + customerno+ "&customername=" + name+ "&customerno=" + no);
...
<body class="easyui-layout layout" style="height: 100%; overflow-x: hidden; border: medium none; margin: 0px; padding: 0px;">
								...
								<table width="100%" cellspacing="0" cellpadding="2">
									<tbody>
										<tr>
											...
											<td><input type="text" form-type="al_textbox" value="" id="CUSTOMERNAME" name="CUSTOMERNAME" style="width:150px;font-size:12px;" class="{required:false}"></td>
											<td><span id="setresult" style="font-size:12px;color:red;"></span><td>
											<td align="right" style="font-size: 12px;">客户编号</td>
											<td><input type="text" form-type="al_textbox" value="" id="CUSTOMERNO" name="CUSTOMERNO" style="width:100px;font-size:12px;" class="{required:false}"></td>
										</tr>
									</tbody>
								</table>
							</form>
								...
								<table cellspacing="0" cellpadding="0" border="0" id="dictionary_list_grid" tabindex="1" role="grid" aria-multiselectable="false" aria-labelledby="gbox_dictionary_list_grid" class="ui-jqgrid-btable" style="width: 100%;font-size: 12px;">
									<tbody>
										...
										<tr class="ui-widget-content jqgrow ui-row-ltr" onmouseover="setBeckGroundYes(this);" onmouseout="setBeckGroundNo(this);">
											<td><input type="radio" value="<s:property value="CUSTOMERNAME"/>//<s:property value="CUSTOMERNO"/>" name="chk_list"></td>
	...
	<div style="display:none;">
		<s:hidden name="customerno"></s:hidden>
