...
.selectCheck {
	...
	$().ready(function() {
		...
		function showQuestion(title, taskid, questionid,xmbh) {
			var pageUrl = "url:zqb_project_question.action?projectNo="
		...
		function showXm(xmbh,instanceid,xmmc) {
			...
			var demid = $("#demid").val();
			if(issx=='1'){
				var pageUrl = "url:openFormPage.action?formid="+formid+"&instanceId="+instanceid+"&demId="+demid;
				$.dialog({
					id : 'projectTask',
					cover : true,
					title : "查看项目",
				...
				var pageUrl = "url:loadVisitPage.action?instanceId="+instanceid+"&formid=91&demId=22&PROJECTNO="+xmbh+"&PROJECTNAME="+xmmc;
				$.dialog({
					id : 'projectTask',
					cover : true,
					title : "查看项目",
		...
		function DwshowXm(lcbh,lcbs,taskid) {
			var pageUrl = "url:loadProcessFormPage.action?actDefId="+ lcbh + "&instanceId="+ lcbs + "&excutionId="+ lcbs +"&taskId="+taskid;
			$.dialog({
				id : 'projectTask',
				cover : true,
				title : "查看项目",
		...
		function search(){
			...
			var valid = mainFormValidator.form(); //执行校验操作
				...
				var seachUrl = encodeURI("zqb_project_xgwthf.action?projectName="
...
<body class="easyui-layout">
	...
	<div region="center" border="false" align="center"
		...
		<form action="zqb_project_xgwthf.action" method="post" name="frmMain" id="frmMain">
		...
		<s:hidden name="issx" id="issx"></s:hidden>
		</form>
		...
		<table WIDTH="100%" style="border:1px solid #efefef">
			...
			<s:iterator value="list" status="status">
				<TR class="cell">
					...
					<TD>
						<a href="javascript:showQuestion('<s:property value='JDMC'/>','<s:property value='KMID'/>','<s:property value='ID'/>','<s:property value='XMBH'/>')">回复</a>
						|
						<s:if test="isDwPj='1'">
							<s:if test="LCBH==null">
								<a href="javascript:showXm('<s:property value='XMBH'/>','<s:property value='INSTANCEID'/>','<s:property value='XMMC'/>')">查看项目信息</a>
							</s:if>
							<s:else>
								<a href="javascript:DwshowXm('<s:property value="LCBH"/>',<s:property value="LCBS"/>,<s:property value="TASKID"/>)">查看项目信息</a>
							</s:else>
						</s:if>
						<s:else>
							<a href="javascript:showXm('<s:property value='XMBH'/>','<s:property value='INSTANCEID'/>','<s:property value='XMMC'/>')">查看项目信息</a>
