...
function doSubmit() {
	...
	if(!isNaN(rzje)){
		if(add=='true'){
			var pageUrl = "xsd_zqb_project_save_rz.action";
			$.post(pageUrl, {projectNo:projectNo,groupid:groupid,rzje:rzje}, function(data) {
				if (data == 'success') {
					W.lhgdialog.tips("保存成功！", 1);
					setTimeout('api.zindex().focus()', 1000);
				}else{
					W.lhgdialog.tips("没有可入账的阶段信息！", 1);
					setTimeout('api.zindex().focus()', 1000);
				}
			});	
		}else{
			var pageUrl = "xsd_zqb_project_edit_rz.action";
			$.post(pageUrl, {projectNo:projectNo,groupid:groupid,rzje:rzje}, function(data) {
				if (data == 'success') {
...
<body class="easyui-layout">
	...
	<div region="center"
		...
		<s:form id="editForm" name="editForm" theme="simple">
						...
						<tr>
							<td align="left">
								<table class="ke-zeroborder" border="0" cellpadding="0"
									...
									<tbody>
										<tr>
											<td class="td_title" id="title_RZJE" width="180">入账金额(万元)</td>
											<td class="td_data" id="data_RZJE"><input type="text"
												id="rzje" name="rzje" style="width:100px;height:22px;" 
												value="${rzje}" />
											</td>
										</tr>
									</tbody>
			...
			<s:hidden name="add" id="add"></s:hidden>
		</s:form>
	</div>
</body>
</html>
