	...
	$(document).ready(function(){
		...
		$("#sch_save").bind("click", function(){
			...
			if(reStartdate==""||reStartdate==null){
				W.$.dialog.alert("请输入开始日期!");
				flag = false;
			}
			if(reMode==""||reMode==null){
				W.$.dialog.alert("请选择周期模式!");
				flag = false;
			}
			if(flag){
		            					...
		            					<tr>
		            						...
		            						<td width="75%">
		            							<div id="reMode0" style="display: block;padding-left: 10px;padding-top:10px">
		            								每<s:textfield name="iworkSchCalendar.reDayInterval" cssStyle="width:20px;"  onkeyup="value=(parseInt((value=value.replace(/\D/g,''))==''||parseInt((value=value.replace(/\D/g,''))==0)?'1':value,10))" onafterpaste="value=(parseInt((value=value.replace(/\D/g,''))==''||parseInt((value=value.replace(/\D/g,''))==0)?'1':value,10))"></s:textfield>天
		            							</div>
		           				...
		           				<legend id="dateTitle"><font color="808080" >重复范围</font></legend>
		            				<table width="100%">
		            					<tr>
		            						<td width="45%">
			            						<label>开始日期:
				            					<s:textfield name="iworkSchCalendar.reStartdate" onclick="WdatePicker({minDate:'#F{$dp.$D(\\'editForm_iworkSchCalendar_reStartdate\\')}',maxDate:'2050-10-01'})" cssStyle="width:100px;" theme="simple">
