...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>
	...
	var api = art.dialog.open.api, W = api.opener;
		...
		//点击事件
			...
			function insertData(treeNode){
				...
				var targetDeptNo = $("#editForm_targetDeptNo").val();
				var targetDeptName = $("#editForm_targetDeptName").val();
				var defaultField = $("#editForm_defaultField").val();
				rebackData(targetDeptId,treeNode.id);
				rebackData(targetDeptNo,treeNode.departmentno);
				rebackData(targetDeptName,treeNode.name); 
				rebackData(defaultField,treeNode.fullname);
				//回调父页面方法
				try{
					parent.selectDeptAddress(defaultField,treeNode);
				}catch(e){} 
				//关闭窗口
				art.dialog.close();
			}
	...
	function beforeAsync() {
		...
		function dosearch(){
			...
			//department_tree_Json.action
			var url = encodeURI("department_tree_g.action?searchOrg="+searchOrg);
		//	var url = "radiobook_search.action?parentDept=<s:property value='parentDept' escapeHtml='false'/>&currentDept=<s:property value='currentDept' escapeHtml='false'/>&startDept=<s:property value='startDept' escapeHtml='false'/>&searchOrg="+searchOrg
			var setting = {
					async: {
						enable: true, 
						url:url, 
