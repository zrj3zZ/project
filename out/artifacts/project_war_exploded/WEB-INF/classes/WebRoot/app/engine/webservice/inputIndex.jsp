...
<html>
  <head>  
	...
	function edit(id,pid){
	     var url = 'web_service_param_editPage.action?inorout=input&id='+id+'&pid='+pid;
		 openDialog('编辑',url);
	}
	function linkSub(id,pid){
	     var url = 'web_service_param_input.action?id='+id+'&pid='+pid;
		 this.location = url;
	}
	function add(){
	    var pid=$('#pid').val();
		var url = 'web_service_param_addPage.action?inorout=input&pid='+pid;
		openDialog('添加',url);
	}
	//添加和编辑窗口
	...
	function del(){
			...
			if(ids.length==0){
			      ...
			      var url='web_service_param_del.action';
	...
	function moveUp(){
	    ...
	    var url='web_service_param_moveUp.action';
	    $.post(url,{id:rows[0].ID,pid:pid,inorout:'input'},function(data){
	         if(data=='ok'){
	             $('#condition_grid').datagrid('reload');
	         }
	    });
	...
	function moveDown(){
	    ...
	    var url='web_service_param_moveDown.action';
	    $.post(url,{id:rows[0].ID,pid:pid,inorout:'input'},function(data){
	         if(data=='ok'){
	             $('#condition_grid').datagrid('reload');
	         }
  ...
  <body class="easyui-layout">
     ...
     <div region="center" style="padding:3px;border:0px;">					
					<div>
						<table id="condition_grid" style="height:400px"  iconCls="icon-edit" singleSelect="false" idField="ID" url="web_service_param_json.action?pid=<s:property value='id'  escapeHtml='false'/>&inorout=input" >
									<thead>
										<tr>
										    <th field="CK" checkbox=true></th>
											<th field="ID" width="50" align="left">序号</th>
											<th field="TITLE" width="100" align="left">参数标题</th>
											<th field="NAME" width="120" >参数名称</th>	
											<th field="VALUE" width="100" align="left" >参数值</th>
