...
function openFormPage(instanceid){
	var demid = $("#Tyxmjdzldemid").val();
	var formid = $("#Tyxmjdzlformid").val();
...
function remove(){
	$.messager.confirm('确认','确认删除?',function(result){ 
		if(result){
			...
			if(ids!=""){
				$.post("deletetyxmjdzl.action",{instanceids:ids},function(data){ 
			    	if(data=='success'){
				   		window.location.reload();
				   	}else{
				   		alert("有"+data+"项删除失败!");
...
function down() {
	...
	if(downs.length>2){
		...
		window.location.href = "downtyxmjdzl.action?downids="+down;
	}
}
function up() {
	var up="";
	...
	if(ups.length>2){
		...
		window.location.href = "uptyxmjdzl.action?upids="+up;
	}
}
function topup() {
	var uptop="";
	...
	if(uptops.length>2){
		...
		window.location.href = "uptoptyxmjdzl.action?uptopid="+uptop;
	}
}
function bottomdown() {
	var downbottom="";
	...
	if(dowbottoms.length>2){
		...
		window.location.href = "downbottomtyxmjdzl.action?downbottomid="+downbottom;
	}
}
function fresh() {
	window.location.href = "sysDem_Mdm_tyxmjdzl.action";
}
function doSearch() {
	var xmlx=$("#xmlx").val(); 
	window.location.href = encodeURI("sysDem_Mdm_tyxmjdzl.action?xmlx="+xmlx);
}
...
<body class="easyui-layout layout" style="height: 100%; overflow: hidden; border: medium none; margin: 0px; padding: 0px;">
				...
				<td style="padding-top:10px;padding-bottom:10px;"> 
					<table width="100%" cellspacing="0" cellpadding="0" border="0"> 
						<tbody>
							...
							<td class="searchdata">
								<input type="text" value="<s:property value="xmlx" />" id="xmlx" name="xmlx" style="width:100px;" class="{maxlength:32,required:false,string:true}">
							</td>
	...
	<div style="padding:5px">
		<div class="ui-jqgrid ui-widget ui-widget-content ui-corner-all" id="gbox_iform_grid" dir="ltr" style="width: 100%;">
					...
					<table cellspacing="0" cellpadding="0" border="0" aria-labelledby="gbox_iform_grid" role="grid" style="width:100%" class="ui-jqgrid-htable">
						<thead>
							<tr role="rowheader" class="ui-jqgrid-labels">
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="" style="width: 5%;">
									<input type="checkbox" class="cbox" onclick="checkAll();" role="checkbox">
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="" style="width:10%;">
									序号
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="" style="width: 10%;">
									项目类型
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="" style="width: 15%;">
									阶段名称
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="" style="width: 50%;">
									资料内容
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="" style="width: 10%;">
									操作
			...
			<div style="position:relative;width:100%; height:480px; line-height: 20px; overflow: auto;">
				<table cellspacing="0" cellpadding="0" border="0" id="iform_grid" tabindex="1" role="grid" aria-multiselectable="true" aria-labelledby="gbox_iform_grid" class="ui-jqgrid-btable" style="width: 100%;">
					<tbody>
						<tr style="height:auto" role="row" class="jqgfirstrow">
							...
							<td style="height:0px;width:5%;"></td>
							<td style="height:0px;width:15%;"></td>
							<td style="height:0px;width:15%;"></td>
							<td style="height:0px;width:50%;"></td>
							<td style="height:0px;width:10%;"></td>
						</tr>
						<s:iterator value="sysdemMdmTyxmjdzlList" status="status">
						<tr class="ui-widget-content jqgrow ui-row-ltr" role="row">
							<td aria-describedby="iform_grid_cb" style="text-align:center;width: 5%;" role="gridcell">
							...
							<td title="<s:property value="#status.count" />" style="text-align:center;width: 5%;" role="gridcell"><s:property value="#status.count" /></td>
							<td title="<s:property value="XMLX" />" style="text-align:center;width: 15%;" role="gridcell"><s:property value="XMLX" /></td>
							<td title="<s:property value="JDMC" />" style="text-align:center;width: 15%;" role="gridcell"><s:property value="JDMC" /></td>
							<td title="<s:property value="CONTENT" />" style="text-align:center;width: 50%;" role="gridcell"><s:property value="CONTENT" /></td>
							<td title="查看详情" style="" role="gridcell">
							<a  onclick="openFormPage('<s:property value="INSTANCEID" />');" href="#">查看详情</a>
							</td>
						</tr>
						</s:iterator>
					</tbody>
	...
	<form action="sysDem_Mdm_xmjdzl.action" method=post name=frmMain id=frmMain>
		...
		<s:hidden name="Tyxmjdzlformid" id="Tyxmjdzlformid"></s:hidden>
		<s:hidden name="Tyxmjdzldemid" id="Tyxmjdzldemid"></s:hidden>
