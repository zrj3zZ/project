...
<link rel="stylesheet" type="text/css" href="iwork_css/jquerycss/icon.css">
	...
	function showItem(id){
		var pageUrl = "process_step_summary_showItem.action?id="+id;
	...
	function add(){
	   var  prcDefId =$("#editForm_prcDefId").val();
				...
				var pageUrl  = "process_step_summary_add.action?prcDefId="+prcDefId+"&actDefId="+actDefId+"&actStepDefId="+actStepDefId;
	...
	function del(){
			...
			if(ids.length==0){
			      ...
			      var url='process_step_summary_del.action';
	...
	function moveUp(){
	    ...
	    var url='process_step_summary_moveUp.action';
	...
	function moveDown(){
	    ...
	    var url='process_step_summary_moveDown.action';
	    $.post(url,{id:rows[0].ID,prcDefId:prcDefId,actDefId:actDefId,actStepDefId:actStepDefId},function(data){
	         if(data=='ok'){
	             $('#showField_grid').datagrid('reload');
	         }
	...
	function moveTop(){
	    ...
	    var url='process_step_summary_moveTop.action';
	    $.post(url,{id:rows[0].ID,prcDefId:prcDefId,actDefId:actDefId,actStepDefId:actStepDefId},function(data){
	         if(data=='ok'){
	            $('#showField_grid').datagrid('reload');
	         }
	...
	function moveBottom(){
	    ...
	    var url='process_step_summary_moveBottom.action';
	    $.post(url,{id:rows[0].ID,prcDefId:prcDefId,actDefId:actDefId,actStepDefId:actStepDefId},function(data){
	         if(data=='ok'){
	             $('#showField_grid').datagrid('reload');
	         }
...
<div region="west" border="false" style="width:180px;padding:3px;border-right:1px solid #efefef">
    ...
    <div region="center" style="padding:3px;border:0px">				
					...
					<div>
						<table id="showField_grid" style="height:auto" style="height:500px" iconCls="icon-edit" singleSelect="false" idField="ID"  url="process_step_summary_json.action?actDefId=<s:property value="actDefId"/>&prcDefId=<s:property value="prcDefId"/>&actStepDefId=<s:property value="actStepDefId"/>" >
									<thead>
										<tr>
										    <th field="CK" checkbox=true></th>
										    <th field="ID" hidden="true" width="50" align="left" >序号</th> 
										    <th field="FORM_TITLE" width="150" >表单标题</th>	 
										    <th field="FIELD_TITLE" width="80" >名称</th>	 
											<th field="FIELD_NAME" width="80" align="left">字段</th>
