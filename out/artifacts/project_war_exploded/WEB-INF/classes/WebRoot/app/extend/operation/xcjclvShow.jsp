      ...
      //查询事件
       $("#search").click(function(){
        ...
        var xcjclx=$("#xcjclx").val();
      	...
      	var pageUrl="xcjclv_search.action";
      	 var seachUrl = encodeURI("xcjclv_search.action?zqdmxs="+zqdmxs+"&zqjcxs="+zqjcxs+"&startdate="+startdate+"&enddate="+enddate+"&xcjcry="+xcjcry+"&xcjclx="+xcjclx);
		...
		function uploadItem(){
			...
			var pageSize=$("#pageSize").val();
			  var zqdmxs = $("#ZQDM").val();
		        ...
		        var xcjcry=$("#xcjcry").val();
		        var xcjclx=$("#xcjclx").val();
			 var seachUrl =encodeURI("xcjcjlToExcl.action?zqdmxs="+zqdmxs+"&zqjcxs="+zqjcxs+"&startdate="+startdate+"&enddate="+enddate+"&xcjcry="+xcjcry+"&xcjclx="+xcjclx+ "&pageNumber=" + pageNumber+ "&pageSize=" + pageSize);
			window.location.href = seachUrl;
...
<div region="north" style="height:50px;font-size:20px;font-family:黑体;padding:5px;border-bottom:1px solid #efefef;" border="false" >
  <div class="tools_nav">
		...
		<tr>
			<td> 
				<label id="operationButton">
					<a id="addAnnouncement" href="javascript:addAnnouncement();" class="easyui-linkbutton" plain="true" iconCls="icon-add">新增检查记录</a>
	...
	<form action="xcjclv_search.action" method="post" name="frmMain" id="frmMain" >
...
<table width='100%' border='0' cellpadding='0' cellspacing='0'> 
	<tr> 
		...
		<td class="searchtitle">现场检查日期 </td> 
		<td id="title_STARTDATE" class="searchdata">
			...
			&nbsp;&nbsp;到&nbsp;&nbsp;<input type='text' onfocus="WdatePicker({minDate:'#F{$dp.$D(\'startdate\')}'})" onchange="checkRQ()" style="width:80px" name='enddate' id='enddate'  value='<s:property value="enddate"/>' >
		</td>
	</tr>
	<tr>
		<td class="searchtitle" style="padding-left:10px;">现场检查人员 </td> 
		...
		<td class="searchtitle">现场检查类型</td>
		<td class="searchdata">
			<select name='xcjclx' id='xcjclx'>
				<option value=''>-空-</option>
				<s:if test="xcjclx=='专项现场检查'">
					<option value='专项现场检查' selected="selected">专项现场检查</option>
					<option value='常规现场检查' >常规现场检查</option>
				</s:if>
				<s:elseif test="xcjclx=='常规现场检查'">
					<option value='专项现场检查' >专项现场检查</option>
					<option value='常规现场检查' selected="selected">常规现场检查</option>
				</s:elseif>
				<s:else>
					<option value='专项现场检查' >专项现场检查</option>
					<option value='常规现场检查' >常规现场检查</option>
...
<tr> 
    	...
    	<table WIDTH="100%" style="border:1px solid #efefef">
			<TR  class="header" style="text-align: center;">
				...
				<TD style="width:6%">证券代码</TD>
				<TD style="width:6%">证券简称</TD>
				<TD style="width:12%">现场检查时间</TD>
				<TD style="width:20%">现场检查人员</TD>
				<TD style="width:20%">现场检查类型</TD>
				<TD style="width:32%">引发现场检查的因素</TD>
			</TR>
			...
			<TR class="cell" >
				...
				<TD style="text-align: center;"><s:property value="sj"/></TD>
				<TD style="text-align: center;"><s:property value="ry"/></TD>
				<TD style="text-align: center;"><s:property value="nsr"/></TD>
				<TD style="text-align: center;"><s:property value="swdjh"/></TD>
				</TD>
