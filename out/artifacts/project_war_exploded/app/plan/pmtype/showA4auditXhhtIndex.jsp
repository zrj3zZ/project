	...
	var selectedNodes;
	    ...
	    function sum(){
			...
			for(var i=0;i<checkeds_BYYS.length;i++){
				...
				sum = sum + checkeds_BYYS[i].value*1;
			}
			$("#subNumTd").text(sum);
		}
		...
		// 更新本月预算值
		function setbyys(id,instanceid,yyye,obj){
			var byys = obj.value;
		...
		function changeView(){	
			...
			var kzbh = ${yskzbh};
			if(type!="project"){
				window.location.href="zhjg_loadZHJGAuditIndex_action.action?yskzbh="+kzbh+"&viewType=project";
			}else{
				window.location.href="zhjg_loadZHJGAuditIndex_action.action?yskzbh="+kzbh;
			}
		...
		function showAuditInfo(instanceid,id){
			...
			var win_width = window.screen.width;
			var page = window.open(url,target,'width=450,height=350,top=10,left=750,location=no,menubar=no,toolbar=no,status=no,directories=no,scrollbars=yes,resizable=yes');
			page.location = url;
...
<!-- TOP区 -->
	<div >
		<div class="tools_nav">
			...
			<table width="100%">
				<TR>
					<td><a href="###" onclick="changeView()" class="easyui-linkbutton" plain="true" iconCls="icon-add">切换</a></td>
				</TR>
			</table>
			</div>
		 </div>
		...
		<table WIDTH="100%" style="border:1px solid #efefef">
			...
			<s:iterator value="list"  status="status">
				...
				<td></td>
					...
					<s:if test="viewType=='project'">
						<TD ><s:property value="XMMC"/></TD>
					</s:if>
					<s:else>						
						<TD ><a href="###" onclick="showht('<s:property value="XMBH"/>')"><s:property value="XMMC"/></a></TD>
					</s:else>
			...
			<s:iterator value="children"  id="inner" >
				...
				<s:if test="viewType!='project'">
					<s:property value="#inner.XMMC"/>					
				</s:if>
				<s:else>
					<a href="###" onclick="showht('<s:property value="#inner.XMBH"/>')"><s:property value="#inner.XMMC"/></a>
				</s:else>
				</TD>
				...
				<TD ><input type="text" name="BYYS" onChange="setbyys(<s:property value="#inner.id"/>,<s:property value="#inner.INSTANCEID"/>,<s:property value="#inner.NDYSYE"/>,this)" id="BYYS_<s:property value="#inner.id"/>" style="width:58px" value="<s:property value="#inner.BYYS"/>"/></TD>
				
				
				<TD style="display:none"><s:property value="#inner.BBZT"/></TD>
				
				<s:if test="#inner.BBZT=='A4'">
				...
				<TD ><a  href="javascript:showAuditInfo(<s:property value="#inner.INSTANCEID"/>,<s:property value="#inner.id"/>)">查看</a></TD>
			</TR>
			</s:iterator>
			</s:iterator>
		</table>
