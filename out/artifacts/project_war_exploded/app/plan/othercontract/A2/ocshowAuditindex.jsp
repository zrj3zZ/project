...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>     		
	...
	var selectedNodes;
		...
		function bohui(id){
			...
			var pageUrl = "oc_getOpinion_action.action?instanceid="+instanceid+"&rowid="+id;
		...
		function setbyys(id,obj){
			...
			var num = byys;
			var instanceid = $("#instanceid").val();
			var pageUrl="oc_byysOcMxAudit_action.action";
		...
		function audit(){
			...
			var url = "oc_aduitOcMx2_action.action?instanceid="+instanceid+"&ids="+ids.join(',');
		...
		// 流程中判断是否有未填写的内容
		function vaData(){
			var checkeds_YSKMBH = $("select[id*='YSKMBH']");
			for(var i=0;i<checkeds_YSKMBH.length;i++){
				if(checkeds_YSKMBH[i].value=='0'){
					alert("提交审批项目中预算科目未选择！");
					checkeds_YSKMBH[i].focus();
					return false;
				}
			}
			...
			var checkeds_FKLY = $("select[id*='FKLY']");
			for(var i=0;i<checkeds_FKLY.length;i++){
				if(checkeds_FKLY[i].value=='0'){
					alert("提交审批项目中付款理由未选择！");
