	...
	var api = art.dialog.open.api, W = api.opener;
		...
		function initTree(){
			var setting = { 
						async: {
							enable: true, 
							url:"org_station_scope_user_json.action?&insId=<s:property value='insId' escapeHtml='false'/>", 
							dataType:"json",
			...
			var zTree = $.fn.zTree.getZTreeObj("radiotree");
				var nodes = zTree.getCheckedNodes(true);
	 			...
	 			for(var i=0;i<nodes.length;i++){
					...
					if(userlist==''){
						if(confirm('是否要清空所有的用户范围 ')){
							var dept = document.getElementById("userlist");
							dept.value = "";  
							var obj = $('#editForm').serialize(); 
						     $.post("org_station_scope_user_save.action",obj,function(data){
						           if(data!=""){
						...
						var dept = document.getElementById("userlist");
						dept.value = userlist.join(",");  
						var obj = $('#editForm').serialize(); 
					     $.post("org_station_scope_user_save.action",obj,function(data){
					           if(data=="success"){
				                	art.dialog.tips("岗位授权管理范围成功",3);
				            	}else{
				            		art.dialog.tips("范围冲突 ，您设置的 用户["+data+"]已经进行过设置",3);
				            	}
					    });
					}
...
<body class="easyui-layout">
    ...
    <div region="south"border="false" title="已授权用户" style="height:100px;color:#0000ff;padding:5px;">
