...
function openFormPage(instanceid){
	var demid = $("#xmjdzldemid").val();
	var formid = $("#xmjdzlformid").val();
...
function remove(){
	$.messager.confirm('确认','确认删除?',function(result){ 
		if(result){
			...
			if(ids!=""){
				$.post("deletexmjdzl.action",{instanceids:ids},function(data){ 
			    	if(data=='success'){
				   		window.location.reload();
				   	}else{
				   		alert("有"+data+"项删除失败!");
...
function down() {
	...
	if(downs.length>2){
		...
		window.location.href = "downxmjdzl.action?downids="+down;
	}
}
function up() {
	var up="";
	...
	if(ups.length>2){
		...
		window.location.href = "upxmjdzl.action?upids="+up;
	}
}
function topup() {
	var uptop="";
	...
	if(uptops.length>2){
		...
		window.location.href = "uptopxmjdzl.action?uptopid="+uptop;
	}
}
function bottomdown() {
	var downbottom="";
	...
	if(dowbottoms.length>2){
		...
		window.location.href = "downbottomxmjdzl.action?downbottomid="+downbottom;
	}
}
function fresh() {
	window.location.href = "sysDem_Mdm_xmjdzl.action";
}
...
<body class="easyui-layout layout" style="height: 100%; overflow: hidden; border: medium none; margin: 0px; padding: 0px;">
					...
					<table cellspacing="0" cellpadding="0" border="0" aria-labelledby="gbox_iform_grid" role="grid" style="width:100%" class="ui-jqgrid-htable">
						<thead>
							<tr role="rowheader" class="ui-jqgrid-labels">
								...
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="iform_grid_ROWNUM" style="width: 10%;">
									序号
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="iform_grid_JDMC" style="width: 15%;">
									阶段名称
								</th>
								<th class="ui-state-default ui-th-column ui-th-ltr" role="columnheader" id="iform_grid_CONTENT" style="width: 60%;">
									资料内容
								</th>
				...
				<table cellspacing="0" cellpadding="0" border="0" id="iform_grid" tabindex="1" role="grid" aria-multiselectable="true" aria-labelledby="gbox_iform_grid" class="ui-jqgrid-btable" style="width: 100%;">
					<tbody>
						<tr style="height:auto" class="jqgfirstrow">
							...
							<td style="height:0px;width:60%;"></td>
							<td style="height:0px;width:10%;"></td>
						</tr>
						<s:iterator value="sysdemMdmXmjdzlList" status="status">
						<tr class="ui-widget-content jqgrow ui-row-ltr" role="row">
							...
							<td title="<s:property value="#status.count" />" style="text-align:center;width:10%"><s:property value="#status.count" /></td>
							<td title="<s:property value="JDMC" />" style="text-align:center;width:15%;"><s:property value="JDMC" /></td>
							<td title="<s:property value="CONTENT" />" style="text-align:leftwidth:60%;"><s:property value="CONTENT" /></td>
	...
	<form action="sysDem_Mdm_xmjdzl.action" method=post name=frmMain id=frmMain>
		...
		<s:hidden name="xmjdzlformid" id="xmjdzlformid"></s:hidden>
		<s:hidden name="xmjdzldemid" id="xmjdzldemid"></s:hidden>
