...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>
	...
	$(function(){
			$('#dg').datagrid({
				url: 'web_service_runtime_showjson.action?pid=<s:property value="pid"/>',
				iconCls: 'icon-save',
				width: 735,
				height: 350,
				fitColumns: true,
		...
		function dosearch(){
			...
			var isChecked = 1;
			if($("#isChecked").attr("isChecked")==true){
				isChecked = 0;
			} 
			...
			var pid = $("#pid").val();
			$('#dg').datagrid({url:'web_service_runtime_showjson.action',    
				queryParams:{pid:pid, startdate:startdate,enddate:enddate,isChecked:isChecked}
			});
		}
		function showinfo(id){
			var pageUrl = 'web_service_runtime_showitem.action?id='+id;
  ...
  <s:form name="editForm" theme="simple">		
  	<div region="north"  style="border:0px;height:90px;padding:12px;">
  		<div style="height:50px;border:1px solid #efefef;padding:5px;vertical-align:middle;background-color:#FCFCF4">
     				<table width="100%">
     				<tr>
     					<td><input type="checkbox" name="isChecked" id="isChecked" checked="checked" value ="isChecked"/>调用未成功</td>
     					<td >开始时间</td>
