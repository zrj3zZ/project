...
<title>IWORK综合应用管理系统</title>
	...
	var api = art.dialog.open.api, W = api.opener;
		function doReback(){
			...
			if(resion==""){
				art.dialog.tips("请填写撤销原因",3);
				$("#resion").focus();
				return false;
			}else{
				 $.post('processManagement_doReback.action',$("#editForm").serialize(),function(data) {
				    	if(data=='success'){
				    		art.dialog.tips("任务撤销成功",3);
				    		var win =api.data.win;
				    		win.location = "process_desk_index.action";
				    		api.close();
				    	}else{ 
				    		try{
						    	showSysTips();
...
<body  class="easyui-layout" >
	 ...
	 <fieldset>
			...
			<div style="padding:5px;color:red;line-height:20px;">
				警告：执行任务撤销后，任务将撤销至当前账户，系统将保留撤销原因记录。继续流转时，将按照原流程重新流转
