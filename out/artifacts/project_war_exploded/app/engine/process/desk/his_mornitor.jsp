...
<title>IWORK综合应用管理系统</title>
	...
	$(function(){
				...
				jQuery("#task_list_grid").jqGrid({
					url:'process_desk_finish_json.action?pagetype=2',
					datatype: "json",
					mtype: "POST",
					autowidth:true, 
					shrinkToFit:true,
					colNames:["序号","来自","任务标题","日期时间"<s:if test="orgroleid!=3">,"当前节点","当前办理人"</s:if>,"操作","模型ID","节点ID","实例ID","流转ID","任务ID"],
					colModel:[
				   		{"index":"1","sortable":false,"align":"center","hidden":false,"width":"30","name":"id"},
				   		...
				   		{"index":"8","sortable":false,"align":"left","hidden":true,"name":"actStepDefId"},
		...
		function doQuery(){
			...
			var  url = "process_desk_finish_json.action?from="+from+"&title="+encodeURI(title)+"&isCreate="+iscreate+"&processStatus="+processStatus;
		...
		function rebackStep(id){
			var pageUrl = "processManagement_showStepReback.action?instanceId="+id;
			var obj = new Object();
			obj.dialogName = "rebackWinDiv";
			obj.win = window;
			art.dialog.open(pageUrl,{
					 id:'rebackWinDiv',  
					title:'流程撤销',   
...
<!-- TOP标签区 -->
  <div region="north" style="height:40px;padding:0px;border:0px;">
