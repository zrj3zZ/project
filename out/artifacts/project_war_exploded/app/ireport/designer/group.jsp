...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>
	...
	$(document).ready(function(){
	    $('#group_grid').datagrid({
				...
				var url = 'url:ireport_designer_group_edit.action?id='+rowData.ID+'&reportId='+reportId;
				openDialog('编辑',url);
				}
			});   
	});
	function edit(id,reportId){
	     var url = 'url:ireport_designer_group_edit.action?id='+id+'&reportId='+reportId;
		 openDialog('编辑',url);
	}
	//添加和编辑窗口
	...
	function add(){
		...
		var url = 'url:ireport_designer_group_add.action?reportId='+reportId+'&reportType='+reportType+'&chartType='+chartType;
		openDialog('添加',url);
	}
	function del(){
	        var ids = new Array();
			var rows = $('#group_grid').datagrid('getSelections');
			...
			if(ids.length==0){
				alert("请选择要删除的记录");
			      ...
			      var url='ireport_designer_group_del.action';
	...
	function moveUp(){
	    ...
	    var url='ireport_designer_group_moveUp.action';
	...
	function moveDown(){
	    ...
	    var url='ireport_designer_group_moveDown.action';
	    $.post(url,{id:rows[0].ID,reportId:reportId},function(data){
	         if(data=='ok'){
	             $('#group_grid').datagrid('reload');
	         }
	...
	function moveTop(){
	    ...
	    var url='ireport_designer_group_moveTop.action';
	    $.post(url,{id:rows[0].ID,reportId:reportId},function(data){
	         if(data=='ok'){
	            $('#group_grid').datagrid('reload');
	         }
	...
	function moveBottom(){
	    ...
	    var url='ireport_designer_group_moveBottom.action';
	    $.post(url,{id:rows[0].ID,reportId:reportId},function(data){
	         if(data=='ok'){
	             $('#group_grid').datagrid('reload');
	         }
  ...
  <body class="easyui-layout">
     <div region="center" style="padding:3px;border-top:0px;">					
					...
					<div>
						<table id="group_grid" style="height:auto"  iconCls="icon-edit" singleSelect="false" idField="ID" url="ireport_designer_group_table.action?reportId=<s:property value='reportId'  escapeHtml='false'/>" >
									<thead>
										<tr>
										    <th field="CK" checkbox=true></th>
											<th field="FIELD_NAME" width="200" align="left">字段</th>
											<th field="FIELD_TITLE" width="200" >名称</th>	
											<th field="EDIT" width="139" align="left" >操作</th> 
										</tr>
									</thead>
						</table>					
