...
$().ready(function() {
	...
	function doSubmit(){
		...
		var pageUrl="zqb_batch_template_save.action";
		$.post(pageUrl,{content:content},function(data){
			if(data!='error'){
		...
		<s:form id="editForm" name="editForm" theme="simple">
			...
			<div id="border">
				<table style="margin-bottom:5px;" class="ke-zeroborder" border="0"
					...
					<tbody>
						<tr>
							<td class="formpage_title">模板管理表单</td>
						</tr>
						<tr>
						...
						<tr>
							<td align="left">
								<table class="ke-zeroborder" border="0" cellpadding="0"
									...
									<tbody>
										<tr id="itemTr_1916">
											<td class="td_title" id="title_CONTENT" width="180">模版正文</td>
											<td class="td_data" id="data_CONTENT" colspan="3">
												<div id="DIVCONTENT">
													<div><input type="hidden" value="${CONTENT}" name="CONTENT" class="valid {string:true}" id="CONTENT" size="100"></div>
													<div><button onclick="showUploadifyPageCONTENT();return false;">附件上传</button></div>
													<s:property value="count" escapeHtml="false" />
													</div>
												<script>
													function showUploadifyPageCONTENT(){
														uploadifyDialog('CONTENT','CONTENT','DIVCONTENT','','true','','');
													}
												</script>
											</td>
										</tr>
