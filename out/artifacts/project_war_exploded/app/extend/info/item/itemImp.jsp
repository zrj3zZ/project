...
<link rel="stylesheet" type="text/css" href="iwork_css/item/itemLoader.css"/>
...
<script type="text/javascript" src="iwork_js/item/itemLoader.js"  charset="utf-8" ></script>
...
<script type="text/javascript">
	...
	function doSubmit(){
		...
		var pageUrl="zqb_XPItemImp.action";
		$.myloading({title: "正在处理，请稍等……"});
		$.post(pageUrl,{fileUUID:fileUUID},function(data){
			if(data=='ok'){
				$.myloading("hide");
				alert("导入完毕！");
		...
		<s:form id="editForm" name="editForm" theme="simple">
			...
			<div id="border">
				<table style="margin-bottom:5px;" class="ke-zeroborder" border="0"
					...
					<tbody>
						<tr>
							<td class="formpage_title">信披事项导入表单</td>
						</tr>
						<tr>
						...
						<tr>
							<td align="left">
								<table class="ke-zeroborder" border="0" cellpadding="0"
									...
									<tbody>
										<tr id="itemTr_1916">
											<td class="td_title" id="title_FILEUUID" width="180">导入文件</td>
											<td class="td_data" id="data_FILEUUID" colspan="3">
														<div id="DIVFILEUUID">
															<div><input type="hidden" value="${fileUUID}" name="FILEUUID" class="valid {string:true}" id="FILEUUID" size="100"></div>
															<div><button onclick="showUploadifyPageFILEUUID();return false;">附件上传</button></div>
															<s:property value="count" escapeHtml="false" />
														</div>
															<script>
																function showUploadifyPageFILEUUID(){
																	uploadifyDialog('FILEUUID','FILEUUID','DIVFILEUUID','','true','','');
																}
															</script>
											</td>
										</tr>
