...
<script type="text/javascript" src="iwork_plugs/artDialog/plugins/iframeTools.js"></script>     		
	...
	var selectedNodes;
		...
		function setSubjectList(event, treeId, treeNode, clickFlag){
			...
			showSetList(typeid);  
			var url = "iwork_fi_subject_user_json.action?typeid="+typeid+"&typekey="+typekey;
			var setting = {
					check: { 
						enable: true
					},
					...
					callback: {
						...
						onAsyncSuccess:loadTreeList,
						onCheck:loadTreeOnCheck
					}  
				}; 
			var treeObj = $.fn.zTree.init($("#subject_tree"), setting);
		}  
		function showSetList(typeid){
			var url = "iwork_fi_subject_setlist_Json.action";
				$.post(url,{typeid:typeid,typekey:$("#typekey").val()},function(data){
		...
		function loadTreeList(event, treeId, treeNode, msg){
			expandAll();
			loadTreeOnCheck(event, treeId, treeNode, msg);
		}
	...
	<div region="north" border="false" style="padding:0px;overflow:no;scrolling:no;border-bottom:1px;">
		<div class="tools_nav">
			<div style="text-align:left;padding-right:10px;">
							  	<a href="###" onclick="doSubjectSet()" class="easyui-linkbutton" plain="true" iconCls="icon-edit">授权</a>
			</div>
		 </div>
	</div>
    <!-- 导航区 -->
