...
var clearButton = "<input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 1px; height: 100%; border: 0' onfocus='this.blur()'"+
         " type=button value='清空' onclick='parent.clearDate()'>&nbsp;"
var maxDay = 38;
var colSpan = 6*7-maxDay;
function writeIframe()
{
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         strIframe += "<tr>";
         for(var x=35; x<maxDay; x++) strIframe += "<td class=out id='meizzDay"+ x +"'></td>";
         strIframe +="<td colspan="+colSpan+" class=out align='center'>"+clearButton+"<input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 1px; height: 100%; border: 0' onfocus='this.blur()'"+
...
function writeFullDateIframe()
{
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         for(var x=35; x<maxDay; x++) strIframe += "<td class=out id='meizzDay"+ x +"'></td>";
         strIframe +="<td colspan="+colSpan+" class=out align='center'>"+clearButton+"<input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 1px; width: height: 100%; border: 0' onfocus='this.blur()'"+
         " type=button value='&nbsp; &nbsp; 关闭' onclick='parent.hiddenCalendar()'></td></tr></table>"+
    "</td></tr><tr><td height=20 width=140 bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table border=0 cellpadding=1 cellspacing=0 width=140>"+
    "    <tr><td><input name=prevYear title='向前翻 1 年&#13;快捷键：↑' onclick='parent.prevY()' type=button value='&lt;&lt;'"+
    ...
    with(WebCalendar.iframe)
    {
        ...
        for(var i=0; i<maxDay; i++)
        {
            ...
            WebCalendar.dayObj[i].onmouseover = dayMouseOver;
            WebCalendar.dayObj[i].onmouseout  = dayMouseOut;
            WebCalendar.dayObj[i].onclick     = returnFullDate;
        }
    }
}
...
{
    this.regInfo    = "AWF BPM业务流程应用服务器软件;关闭的快捷键：[Esc]";
    this.regInfo   += "";
    this.daysMonth  = new Array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
    this.day        = new Array(maxDay);            //定义日历展示用的数组
    this.dayObj     = new Array(maxDay);            //定义日期展示控件数组
 ...
 *  调用回显精确时间的函数， 精确到分钟
 **/
function calendarFullDate() //主调函数
{
    var e = window.event.srcElement;   
	writeFullDateIframe();
    var o = WebCalendar.calendar.style; 
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    for(var i=1; i<maxDay-w-WebCalendar.daysMonth[m-1]+1; i++)
    {
        WebCalendar.day[WebCalendar.daysMonth[m-1]+w-1+i] = i +"/"+ (parseInt(m, 10)+1) +"/"+ y;
        if(m==12) WebCalendar.day[WebCalendar.daysMonth[m-1]+w-1+i] = i +"/"+ 1 +"/"+ (parseInt(y, 10)+1);
    }
    for(var i=0; i<maxDay; i++)    //这个循环是根据源数组写到日历里显示
    {
...
function returnFullDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        var returnValue;
