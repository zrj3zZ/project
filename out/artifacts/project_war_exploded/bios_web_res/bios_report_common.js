...
function bios_win(url,w,h,icon){
	biosPopup.alert({message:url,iframe:true,titleBar:false,width:w,height:h,icoCls:icon});
}
...
function bios_setHeight(obj, h){
	obj.style.height = h + 'px';
}

function bios_setWidth(obj, w){
...
//custom ctrl
var bios_customCtrlData;

var bios_customCtrlId;

function bios_customCtrlHandler(tp){
	if(tp=='ok'){
		var rtn = biosPopup.getPage().contentWindow.setCtrlValue();
		document.all[bios_customCtrlId].value = rtn.val;
		document.all[bios_customCtrlId+"_v"].value = rtn.txt;
	}
}

function bios_showCustomCtrl(url,w,h,cid){
	bios_customCtrlId = cid;
	bios_customCtrlData = {};
	bios_customCtrlData.val = document.all[cid].value;
	bios_customCtrlData.txt = document.all[cid + "_v"].value;
	var addParam = "ctrl_val=" + bios_customCtrlData.val + "&ctrl_txt=" + bios_customCtrlData.txt;
	if(url.indexOf("?")>=0)
		url += "&" + addParam;
	else
		url += "?" + addParam;
	biosPopup.alert({message:url,titleBar:false,width:w,height:h,icoCls:null,iframe:true,btn:["OK", "CANCEL"],handler:bios_customCtrlHandler});	
}
